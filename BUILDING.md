...
You need Python 3.9+ and a stable Rust toolchain.
...
3. Assembly of `dist/portable/Unlatched/`: the desktop exe renamed to
   ...
   also zipped to `dist/Unlatched-0.1.1-portable-win64.zip` for anyone
...
4. If `ISCC.exe` can be found (a standard install location, or the
   ...
   into `dist/Unlatched-Setup-0.1.1.exe`. If it cannot be found, the
