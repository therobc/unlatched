...
re-advertise. Not one board in the collection reuses the old one - Old
Dominion, RXO, Clayton Homes, VillageMD and NICE all issue a fresh id. So
...
---------------------------------
The first measurement of this grouped on company+title and reported that Old
Dominion re-advertised "Class A CDL Local Driver" on 18 separate dates. That
was wrong: Old Dominion has terminals in dozens of cities and each posts its
own opening. Adding LOCATION separates breadth from repetition. Same company
...
#
# Decided 2026-08-12: "Reposted jobs usually don't have long wait times in between.
# If a job gets reposted more than 4 weeks later, treat it as a new entry and
# link the original job or group that it originated from."
#
# MEASURED ON THE OPERATOR'S BOARD BEFORE BUILDING THIS, 2026-08-13, over 9,161
...
# Veeva seat re-advertised after 1,788 days was being described as "the earlier
...
class Repost(NamedTuple):
    ...
    def real_gaps(self) -> list[int]:
        ...
        testing: an Included Health seat went 41 days and then 3, and reading
        only the last gap called the whole history a re-listing - hiding the
        41-day turnover that is the thing worth knowing.
        """
    ...
    def summary(self, key: str | None = None) -> str:
        ...
        was what made a five-year-old Veeva seat read as a failed search.
