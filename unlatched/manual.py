...
from . import db, discover, screen
...
# original reasoning was sound when it was written (decided 2026-08-09): no
# board was watching those rows either, so nothing else could tell the person
# the posting had gone.
#
...
# all 334 rows this button would have fetched had arrived from a collector,
# and not one was hand-added. It fetched nothing only because
# fetch.read_added_links ships ON since 2026-09-07 - and that setting has to stay ON for
...
#
# A CATEGORY, not a site. One host is the only member today and is what this
# path is tested against - but nothing here is about that host, and naming the
# category after it would say there is an integration where there is only a
# rule (decided 2026-08-08).
#
...
# person-present, one-page-at-a-time path (decided 2026-08-06). Reading the one
# page somebody is looking at, so they can log the job they are about to apply
# for, is not crawling.
#
# WHAT WAS MEASURED BEFORE BUILDING IT, on 2026-08-06, against that host:
...
# It ships ON, because every read behind it is a person's click: adding a
# link, or pressing the re-check button. Nothing scheduled reaches here.
...
# The attended-only host's public job page carries no schema.org markup, so
# these lift the same three facts out of the page's own containers. CSS class
# names are a far weaker contract than markup: a restyle breaks them, and it
# breaks them SILENTLY, which is why read_attended_page returns {} and the
# person types instead of the app inventing anything.
...
def read_attended_page(html: str) -> dict[str, str]:
...
def read_posting(url: str, *,
    ...
    if stat != 200 or not html:
        return {}
    if attended:
        return read_attended_page(html)
...
def add(con: sqlite3.Connection, cfg: dict[str, Any], url: str, *,
    ...
    # to be enforced rather than observed (decided 2026-08-08).
    ...
    # THE BOARD BEHIND THE LINK, when the link says which.
    #
    # Adding a job by hand used to record the employer with no board at all,
    # so the next collect skipped them and the person came back to paste the
    # next posting from the same company by hand. One paste, one job, for
    # ever.
    #
    # But the link people actually have IDENTIFIES the board: an aggregator's
    # "original job post" goes to /embed/job_app?for=<slug>, and the slug IS
    # the collector's reference. Reading it turns one paste into "this
    # employer, from now on" - every posting they publish, direct from their
    # own board, without going back to whoever pointed here.
    #
    # ONLY WHEN THE LINK SAYS SO. No fingerprint means no board is recorded
    # and behaviour is exactly as before: a guess here would point the
    # collector at somebody else's slug and quietly file their jobs under
    # this employer.
    board, board_ref = "", ""
    for hit in discover.detect_ats(url):
        parts = [p for p in hit.get("parts") or [] if p]
        if parts:
            board, board_ref = hit["provider"], parts[0]
            break

    company_id = db.upsert_company(
        con, company or "Added by hand",
        ats=board, ats_ref=board_ref,
        # "yielding" is what a resolved board is called everywhere else, and
        # the Companies screen reads it to say whether an employer can be
        # collected from. Saying "added by hand" while holding a working
        # board would report the employer as unreadable.
        probe_status="yielding" if board else "added by hand",
        origin=db.MANUAL)
...
# the boards; it does not re-read the same hand-added page five times.
...
def due_rows(con: sqlite3.Connection, now: datetime | None = None,
    ...
    yet. Two exclusions, both decided 2026-08-09:
    ...
    ANYTHING WITH A STATUS. Applied, Pass, No Offer, Interviewed, Offer, Hired -
...
def recheck(con: sqlite3.Connection, cfg: dict[str, Any], *,
    ...
    gone, unreadable = [], []
    # Every row a request was actually made for, whatever came back.
    # NOT len(gone), which is what this used to report: a run where all
    # 50 links were read and every one was still open counted zero, so
    # the healthiest possible outcome printed as "checked 0".
    checked = 0
    ...
    for row in due_rows(con, now, limit=limit,
        ...
        # Counted here rather than at the top of the loop: a row filtered
        # out by may_fetch above never became a request and was not
        # checked.
        checked += 1
    ...
    return {"checked": checked, "gone": gone,
