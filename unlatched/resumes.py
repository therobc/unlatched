...
version - which is the workflow the first user described: search, optimise, search again
...
def active_path(cfg: dict[str, Any],
    ...
    The OPTIMISED copy when one exists, because that is the document being
    sent; otherwise the original. `resume_path` in config is still honoured
    last, so a profile set up before any of this existed keeps working
