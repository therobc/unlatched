...
DEFAULTS: dict[str, Any] = {
    "search": {
        ...
        "title_exclude": [],
        "seniority": [],
        # Places the person can actually work, e.g. ["Dayton, OH",
        # "Springfield, OH"] or a bare state ["OH"]. Only applied to postings
        ...
        # match: shown, flagged, and distinguished from a clean keep. Verified
        # by construction in screen.py's _salary_gate/alt_floor handling. Null
        ...
        # dropped - verified by construction in screen.py's employment_mod.accepted
        # handling, which sets `alt = True` rather than `qualified = False` -
        # it is flagged "alt" so the person can dismiss it in
        ...
        # Hybrid is its own answer rather than a shade of remote - verified by
        # construction in screen.work_mode, which tests HYBRID_DECLARED
        # first: a hybrid
        ...
        "work_modes": [],
        # Superseded by work_modes, and read only when work_modes is empty -
        # verified by construction in config_draft.rs's has_mode and in
        # screen.wanted_modes, both of which check work_modes first - so
        # a config written before the three ticks existed keeps its search.
        "remote_scope": "any",
    },
    "skills": [],
    "resume_path": None,
    # Which attached copy to screen against, by file name.
    # Empty means "whichever is newest" - see resumes.active_path.
    "resume_pinned": "",
    "sources": {
        "greenhouse": True,
        "lever": True,
    ...
    # category (decided 2026-08-08). One site is not the category. It is the
    ...
    "fetch": {
        ...
        #
        # SHIPPED OFF (decided 2026-08-08), and the reason is about what an
        # author publishes rather than about whether the behaviour is
        # defensible. Reading one page a person is looking at is not crawling -
        # that argument stands - but "a user turned that on" is a materially
        # different position from "the author shipped it on".
        #
        ...
        # ON by default since 2026-09-07. A link somebody added by
        # hand should arrive with its details filled in, and a
        # re-check should be able to say whether the posting is
        # still up - neither of which this setting off can do.
        #
        # SAFE TO DEFAULT ON ONLY BECAUSE EVERY READ IS A CLICK.
        # `recheck` is reachable from the CLI subcommand, the
        # Collect menu entry and the triage button, and nothing
        # else - refresh.py and collectors.py never call it. That
        # is what keeps an attended-only host attended.
    ...
    #
    # SHIPS OFF, and null IS the off switch - verified by construction in
    # collectors.py, which reads cfg['ingest']['path'] as a legacy
    # single-collector fallback only when it is set. Kept rather than cut
    # because the code is small, it is exercised by the test suite either
    # way, and removing a feature to re-add it later
    ...
    # then rename - or a refresh landing mid-write reads half a file. By
    # definition of POSIX rename semantics, the same
    ...
    "profile": {
        ...
        # works without a separate alias table - verified by construction in
        # requirements.compare's license handling.
        "licenses": [],
        ...
        # posting stating that requirement - verified by construction in
        # screen.py's `if profile.get("clearance_ok") is False:` /
        # `if profile.get("public_trust_ok") is False:` checks.
        "clearance_ok": None,
...
#
# "onsite_ok" USED TO BE ACCEPTED HERE AND DID NOTHING. It appeared exactly
# once in the whole codebase - in this set - so it passed validation and then
# fell through to "any" behaviour everywhere. Named beside "remote_only" it
# reads as its opposite, so somebody picking it to mean "onsite roles please"
# got an unfiltered search and no indication why. Saying the value is not
# recognised is better than accepting it and ignoring it. What they want is
# search.work_modes = ["onsite"], which is implemented, tested, and a tick box
# on the Config screen.
VALID_REMOTE_SCOPE = {"any", "prefer_remote", "remote_only"}
# Kept in step with screen.WORK_MODES, which is where the meaning lives.
VALID_WORK_MODES = {"remote", "hybrid", "onsite"}


def defaults() -> dict[str, Any]:
    # By definition of the stdlib typeshed stubs, json.loads() is typed Any
    # since its return shape
...
def validate(cfg: dict[str, Any]) -> list[str]:
    """Return a list of problems (empty = ok) - verified by construction,
    no raise statement appears anywhere in this function's body. Never
    raises because config is user
...
# the clear and `save` protects them on the way out - verified by
# construction: load() below calls _map_secrets(..., keystore_mod.unprotect)
# and save() calls _map_secrets(..., keystore_mod.protect) - so every
# caller in the package works with a plain string and only the file on
# disk is wrapped.
...
# whatever the default happens to be. Somebody who had deliberately turned a
# fetching setting OFF would find it back ON after an update, which is the
# worst direction for that kind of setting to move on its own.
#
# Applied IN ORDER - verified by construction in the `for old, new in
# RENAMED_KEYS:` loop below, which walks the tuple sequentially - so a
# chain works: a config still carrying the original
# name is walked forward one hop at a time to the current one.
#
# EMPTY, and verified as such two lines below (the literal `()`).
# Believed, not measured, that every entry it once held was a legacy
# spelling from before the first public release - no history under
# app/ to check that against. The mechanism stays for the first rename
# that comes after.
