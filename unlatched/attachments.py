...
Those two are treated differently, and only in one direction. Decided 2026-08-12:
"That's the only place I am currently wanting to restrict attachment access.
Everything else needs to be AI accessible like resumes for writing." An
assistant on this machine helping tailor a resume is the point of the app
...
# EVERY FILE IS DOWNLOAD-ONLY (Decided 2026-08-13: "Build attachments as download
# only"). The kinds below decide an icon and a hover, never a renderer: the app
# opens nothing, so it carries no decoder or parser for any format at all.
