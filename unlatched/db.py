...
# Aliased because `status` would otherwise be a local variable name in
# some functions here, believed not measured for the current file - a
# module shadowed by a parameter fails at the call rather than at import,
# by definition of Python's scoping rules.
...
# Columns added after the first release. By definition, CREATE TABLE IF
# NOT EXISTS does nothing to a table that already exists, so an
# installed database keeps its original shape forever unless it is
# migrated - verified by construction in _migrate_jobs below, which
# ALTERs in exactly this tuple.
ADDED_JOB_COLUMNS = (("source", "TEXT"), ("employment_type", "TEXT"),
                     ...
                     # be the same job - verified by construction: dupes.py uses
                     # apply_url as its join key, the only field that says so exactly
                     # rather than by text similarity.
                     ("apply_url", "TEXT"),
                     # The posting this one duplicates, and why. A COLUMN
                     # rather than a deletion: verified by construction in dupes.py, which
                     # sets duplicate_of via UPDATE and excludes it with
                     # `duplicate_of IS NULL` rather than deleting rows, so the row keeps
                     # its status and its history and an over-fire is one UPDATE from
                     # being undone.
                     ("duplicate_of", "TEXT"),
                     ...
                     # one". Believed, not measured, that a sending collector hit exactly
                     # this and three runs of rows arrived unjoinable - there is no history
                     # under app/ to check that incident against.
                     #
                     ...
                     # which apply_url should carry), or empty (unknown) - verified by
                     # construction against importer.py's APPLY_KINDS = ("easy-apply",
                     # "external").
                     ("apply_kind", "TEXT"),
...
# starter.seed() and rediscover BOTH can write probe_status="yielding"
# (verified at starter.py:216 and rediscover.py:141), so a shipped
# employer and one the app found by crawling can be indistinguishable -
...
def _migrate_jobs(con: sqlite3.Connection) -> None:
    ...
    if added and "source" not in existing:
        ...
        # recoverable exactly, not guessed. Verified by construction in the
        # substr()/instr() SQL immediately below.
        con.execute(
    ...
    # Verified by construction: reposts.backfill_seats's own docstring
    # states "Runs over history as well as new rows".
...
def connect(home: str | os.PathLike[str] | None = None) -> sqlite3.Connection:
    ...
    # table that references one, so those tables have to exist first. Verified
    # by construction: connect() above calls this only after executescript,
    # _migrate_jobs, _migrate_status_tables and commit. It takes
...
def upsert_company(con: sqlite3.Connection, name: str, *, domain: str = "",
    ...
    # id is companies.id, an INTEGER PRIMARY KEY - by definition of the
    # stdlib typeshed stubs, sqlite3.Row.__getitem__ is typed Any since a
    # row's column types are not known statically; int() states the schema
    # fact the stubs cannot.
    ...
    if row:
        ...
        if sets:
            ...
            # never from caller input, so there is no injection surface. Verified
            # by construction: `sets` above is built only from the literal column
            # names domain/careers_url/ats/ats_ref/probe_status.
    ...
    # the table has no rowid - by definition of sqlite3's own documented
    # behaviour, and verified by construction: neither is true for this
    # fixed INSERT into a normal rowid table two lines above, so a None
    # here means sqlite itself is broken.
...
def upsert_job(con: sqlite3.Connection, key: str, fields: dict[str, Any]) -> None:
    """Write to jobs.* ONLY - verified by construction in the SQL below.
    Never touches job_status or job_status_log -
    ...
    # The store boundary for links (found by a red-team review, dated at
    # unlatched/links.py:15 as 2026-08-08). A posting's `url`
    ...
    if exists:
        ...
        if cols:
            ...
            # attacker-reachable. Verified by construction two lines above:
...
def list_jobs(con: sqlite3.Connection, *, qualified_only: bool = False,
    ...
    if not include_closed:
        ...
        # interpolated here is the run of "?" markers to bind them to. Verified
        # by construction: status_vocab.placeholders is tested by
        # test_a_status_list_becomes_bind_markers_not_text_in_the_statement to
        # emit exactly one "?" per value. S608 is
...
def mark_delisted(con: sqlite3.Connection, company_id: int, seen_at: str) -> int:
    ...
    """
    # A hand-added job is never delisted by this - verified by construction
    # in the query below (`AND source != 'manual'`). It came from a link the
    ...
    #
    # THE KEYS ARE READ BEFORE THE UPDATE (verified by construction: the
    # SELECT below runs before the UPDATE) so the same rows can be handed to
