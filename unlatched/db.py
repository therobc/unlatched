...
CREATE TABLE IF NOT EXISTS jobs (
  ...
  -- weeks later (reposts.NEW_ENTRY_DAYS). Decided 2026-08-12: "If a job gets
  -- reposted more than 4 weeks later, treat it as a new entry and link the
  -- original job or group that it originated from."
  --
...
ADDED_JOB_COLUMNS = (("source", "TEXT"), ("employment_type", "TEXT"),
                     ...
                     # one". The collector author hit exactly this on her own side - a silent
                     # empty write looked identical to a job with no external
                     # route, and three runs of rows arrived unjoinable with
                     # nothing to say so.
                     #
                     # She now classifies at collection and asked to be told if
                     # a row ever arrives with neither a destination NOR a
                     # classification. That request was unimplementable here:
                     # there was nowhere for the classification to live, so the
                     # audit she wanted could not be run at all.
                     #
...
#
# It could not be answered before, and the question is not academic: the first user asked
# for a "refresh seeded companies" action, which cannot be expressed without
# it. starter.seed() and discover BOTH write probe_status="yielding", so a
# shipped employer and one the app found by crawling are indistinguishable -
...
def retire(con: sqlite3.Connection, keys: list[str], *, at: str) -> int:
    ...
    HIDES, does not delete (the first user asked for delete; this is the same result with
    a way back). Everything the row carries is kept: the status they set, the
