...
Unlatched does not crawl the aggregators - Indeed, Glassdoor, LinkedIn - and
should not. This module is what makes that a POSITION rather than a limitation: the
...
WE PULL. THEY NEVER PUSH (decided 2026-08-12). One program writes to this
database and it is this one. A collector that exports a file needs no access to the
...
class Collector:
    ...
    @property
    def taken_marker(self) -> str:
        """The meta key holding WHEN a file from this collector was last taken
        in.

        DISTINCT FROM BOTH OF THE OTHERS, and the distinction is the whole
        point: `marker` is the fingerprint of the last file taken (what, not
        when), `seen_marker` is when we last LOOKED (which advances even when
        nothing arrived). Neither answers "does this app have the sender's
        latest", which is the question the dashboard asks.
        """
        return f"ingest_taken:{self.id}"
...
def migrated_from_ingest_path(cfg: dict[str, Any]) -> list[Collector]:
    ...
    daily; an upgrade that silently stopped their handoff would not be visible
