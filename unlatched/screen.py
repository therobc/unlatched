...
stays out of the agent's hands.
...
# Positive evidence a posting is remote. Believed, not measured, that a
# posting which simply never mentions location was once being treated
# as remote - no history under app/ to check that against. Silence is
# not evidence, so the gate below only
...
#
# Every branch requires "hybrid" NEXT TO a word about working, verified
# by construction in the pattern below, because
...
def work_mode(title: str, location: str, description: str) -> tuple[str, str]:
    ...
    if is_remote:
        ...
        # Verified by construction: the ONSITE_DECLARED check two lines below
        # runs even after is_remote is already true.
    ...
    if m:
        return "onsite", f"description: {m.group(0)}"
    # The default. Believed, not measured, that most onsite postings never
    # say so - because to the employer
...
# compensation. The salary parser needs a guard on the surrounding context
# before it accepts a number as a salary at all.
...
def title_wants(term: str, title: str) -> bool:
    ...
    Somebody who types "HR Specialist" means it: they will take "HR
    Onboarding Specialist" and "HR Operations Specialist" too. Requiring the
    words to sit next to each other rejected exactly those postings and left a
    real candidate with nothing, which no user would read as anything but the
    tool being broken.

    EVERY WORD OF THE TERM MUST STILL APPEAR. Acronyms are not expanded, so
    "HR Specialist" does NOT match "Human Resources Specialist" - "HR" is not
    a word in it. Somebody who wants both spellings puts both in
    search.title_include, which takes as many terms as they like.
    ...
    # "help" inside "Helpdesk" - verified by construction in
    # _spelling_variant below. Believed, not measured, that a real
    # "IT HelpDesk Analyst" sat uncollected in the corpus for exactly this
    # reason - no history under app/ to check that against.
    return _spelling_variant(term).search(title) is not None


def title_may_pass(title: str, title_include: list[str] | None) -> bool:
    """Could this title survive search.title_include?

    Used by collectors that decide whether a posting is worth a DETAIL
    REQUEST before making one. The title is already on the list page, so the
    decision can be made before spending anything.

    NO FILTER MEANS EVERYTHING PASSES, which keeps the old behaviour exactly
    for a profile that has not set one, and for every caller that does not
    pass the argument at all.

    IT WRAPS title_wants, the same matcher screening itself uses, so a
    pre-filter cannot start disagreeing with the gate it is anticipating. A
    looser rule here would only waste requests; a stricter one skips a
    posting that goes on to qualify, which is the failure that matters.

    IT LIVES HERE RATHER THAN IN A COLLECTOR because two collectors need it
    and the first version of that was two different rules: oracle_hcm used
    title_wants while sitemap.py used a plain substring test, which is
    stricter, so "HR Specialist" stopped matching "HR Operations Specialist"
    on one of the two paths and the postings were dropped unseen.
    """
    if not title_include:
        return True
    return any(title_wants(term, title or "") for term in title_include)
...
def _word_or_plural(word: str, title: str) -> bool:
    ...
    job, and a search asking for the plural against a posting written in the
    singular scored "title matches none of search.title_include" - which is
...
def _spelling_variant(term: str) -> re.Pattern[str]:
    ...
    if len(squashed) < _MIN_VARIANT_LEN:
        # Never matches, by definition: `(?!x)x` requires the next character to
        # both not be x (the lookahead) and be x (the literal), which no
        # character can satisfy - the caller has already tried the whole-word
        # rule, and a short term gets nothing further.
...
def _salary_gate(salary_max: int | None, floor: float | None) -> tuple[bool, str]:
    """Judge the floor against the range TOP - verified by construction,
    the comparison below reads salary_max, never a low/min value.
...
def screen_job(job: Any, cfg: dict[str, Any], resume_text: str = "") -> dict[str, Any]:
    ...
    # count. Verified by construction in the alt/alt_reason assignments
    # below.
    alt = False
    # WHICH shortfall put the row in the alt pile, for the two cards that
    # separate them. Assigned with `or`, so the FIRST one found wins and a
    # later condition never renames it: pay is checked before the profile
    # requirements, and a job that is both underpaid and a poor fit is one a
    # reader dismisses on the money. Tested on a posting that trips both, by
    # test_verdicts::test_pay_wins_when_a_job_falls_short_on_both.
    alt_reason = ""
    ...
    # A job you cannot get to is not a job you can take. Verified by
    # construction: the `if not is_remote:` guard below is what lets
    # remote roles skip this entirely; there is no commute to judge -
    # hybrid roles do NOT, because hybrid means going in.
    if not is_remote:
    ...
    # Vetting the candidate has ruled out. Verified by construction: this is
    # the first call to requirements_mod in this function - screening
    # judged the SEARCH (title, place, pay) and left what a posting
    # demands of the reader as display-only information. A clearance the
    # candidate cannot get is a hard disqualifier, not a footnote, so it
    # belongs here.
    ...
    if not ok:
        ...
        if alt_floor and salary["high"] is not None and salary["high"] >= alt_floor:
            alt = True
            alt_reason = alt_reason or "salary"
    ...
    # location - verified by construction in country.accepted, which
    # defaults to True (domestic) unless is_foreign finds evidence - so a
    # posting naming no place at all is never discarded on suspicion.
    ...
    # Employment type. A mismatch flags rather than drops - verified by
    # construction two lines below (`alt = True`, not `qualified = False`).
    # See employment.accepted for why a person dismissing a job beats never
    # seeing it.
    ...
    if not ok:
        alt = True
        alt_reason = alt_reason or "requirements"
        reasons.append(why)
    ...
    # actually take this job". Nothing here drops a posting outright: verified
    # by construction below, this branch only ever sets `alt = True`, never
    # `qualified = False` - so a disqualifier flags the row as alt
    ...
    score = 60.0
    # THE SAME MATCHER THE GATE USED, verified by construction: title_wants
    # below is the same function _title_screen calls above. Believed, not
    # measured, that a substring test stood here before, and that a real
    # "IT HelpDesk Analyst" against "help desk" once qualified and scored
    # 60 where an exact phrase scored 80 - no history under app/ to check
    # that against.
    if title and any(title_wants(term, title)
                     for term in (search.get("title_include") or [])):
        score += 20.0
    ...
    return {
        ...
        # Always written, including as "" - verified by construction: `asks`
        # is initialised above and this key is always present in the dict
        # below regardless - a row that stops qualifying must
