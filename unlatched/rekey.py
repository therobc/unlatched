...
Decided 2026-08-13: "Get it right. I don't care if you have to move the pipeline
to make the data reflect accurately."
...
def backup(home: Path | os.PathLike[str] | str, con: sqlite3.Connection) -> Path:
    ...
    target = Path(home) / f"unlatched.db.before-rekey-{stamp}"
    with sqlite3.connect(target) as dest:
        con.backup(dest)
    dest.close()
    if not target.is_file() or target.stat().st_size == 0:
        msg = f"backup at {target} was not written - refusing to rekey"
