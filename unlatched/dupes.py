...
another: a listing that routes to apply.workable.com is the same job as the
...
Measured across 38 republisher rows against 344 employer rows: the one
true duplicate scored 0.58 on description similarity - UNDER a 0.75
...
that lesson once: keying reposts on company + title collapsed 18 separate
terminals of one freight carrier into one phantom history. A missed duplicate
costs one wasted read. A false merge HIDES A JOB somebody wanted and they never
learn it existed. Hence title agreement on the fuzzy path, and grouping rather
than deletion.
"""
...
#
# Measured against a real corpus rather than chosen: `dedupe --measure` prints
# the distribution it was read from, so the number can be checked rather than
# taken on trust. It sits above the level I / level II and region-variant
# pairs, which share ~84% of their text and are genuinely separate
# requisitions.
...
def title_agrees(left: str, right: str) -> bool:
    ...
    Deliberately strict. 12 near-identical pairs in a 372-row validation
...
def _a_separate_round(one: dict[str, Any], other: dict[str, Any]) -> bool:
    ...
    The rule, 2026-08-12: a job reposted more than four weeks later is a NEW
...
# Sites that keep their own record of what you applied to. When the same job
# is on one of these AND on an employer's board, the row worth keeping is
# this one: applying here leaves an audit trail somewhere other than this
# app, and the employer's board has no idea you ever went through it.
#
# A LIST WITH ONE MEMBER IS STILL A LIST. The property is what decides, not
# the name, and nothing else about the app changes if another site earns a
# place here.
...
#
# This exists because an earlier change could not fire at all. apply_url is
# written only by `add` and `import`, so on a real board 166 rows of 7,484
# carried one - and every one of those was a row somebody had pasted in or
# imported. The join had one side and could never match. Measured 2026-08-09:
# normalising these sources' posting URLs the same way surfaced 11 genuine
# cross-board pairs that had been invisible, across Greenhouse, Ashby and
# Workday - each an imported row and a collected row for one requisition.
#
...
# KEYED ON SOURCE - the collector that produced the row - NOT on host. It was
# read as a host list once (2026-08-09) and answered with 21 ATS platforms
# seen in real apply_urls. Most of them cannot apply: a name here only matches if
# Unlatched HAS a collector emitting it, so listing iCIMS while nothing collects
# iCIMS adds a row that can never join. That list did surface two real misses
...
# There are no such collectors: they were plausible ATS names written instead
# of read off sources.registry(), and they would have sat here looking like
# coverage. test_every_listed_source_exists now makes that impossible.
APPLICATION_IS_THE_POSTING = (
    ...
    # sitemap. One handoff carried 31 rows across 24 single-tenant hosts -
    # one per employer, each on its own domain - which is a shape no named
    # host list can ever scale to. Keying on the SOURCE sidesteps
    # that entirely: however many employers there are, they arrive through
...
def _is_imported(row: dict[str, Any]) -> bool:
    """Is this the side worth keeping - see PREFERRED_HOSTS.

    Decided from the POSTING URL, not from the source label. The first attempt
    read every "manual" row as preferred, which made every hand-added job -
    from any site at all - look like the preferred route. Both rows in a pair
    then looked the same, and the rule silently did nothing at all. Caught by
    looking at the grouped view and finding the wrong side folded away.
    """
...
def _has_history(row: dict[str, Any]) -> bool:
    ...
    no offer still represents an application that was made.
    """
...
def _same_employer(a: dict[str, Any], b: dict[str, Any]) -> bool:
    ...
    Loose on purpose - one side may write "Northwind" and the other "Northwind
    Health, Inc." - because the question being answered is only "is this an
...
def _primary(a: dict[str, Any], b: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    A PREFERRED_HOSTS ROW WINS when the same job exists on both sides
    (decided 2026-08-09). Not because it is a better route - it usually
    forwards to the same ATS - but because such a site records the
    application in its own tracker, which is an audit trail the person gets
    for free. Two applications have been recovered ONLY because that record
    was still visible after this app had lost it.

    EXCEPT WHEN THAT ROW IS A REPUBLISHER, which matters: a republisher's
    listing is not a preferred route, it is an intermediary, and for the
    staffing agencies among them it carries first-touch risk - an employer
    dropping a candidate rather than settle a fee dispute over who sourced
    them. A different employer named on the two rows is the signal, so in that
    case the employer's own posting is kept.
    ...
    """
    # HISTORY OUTRANKS EVERYTHING, including the preference above.
    #
    # If somebody has already applied through the board row and the preferred
    ...
    # sits in the grouped view. That is precisely the failure that has lost
    # applications before - status recorded on one surface and invisible on
    # the one being worked from.
    #
...
def find(con: sqlite3.Connection,
    ...
    jobs:
      ...
      one bank's Wealth Management Client Associate, at three branches
      the same bank's Relationship Banker, at two more
      a Client Relationship Consultant 3 at two branches of one city
    ...
    This is the freight-terminal over-fire again, at scale: keying on
    company+title once collapsed 18 separate terminals into one phantom
    history. The rule stands - a missed duplicate costs one wasted read, a
    false merge hides a job and nobody ever learns it existed.

    So the apply destination carries this feature. It is exact, it is what
    the measurement pointed at, and it is the only signal in the corpus
...
def rebalance(con: sqlite3.Connection) -> list[tuple[str, str]]:
    ...
    invisible: an advert and an ATS requisition are grouped, the advert is
    kept because applying there gives a free audit trail, and then it expires.
    Adverts come down well before the requisition behind them does. The person
    is now looking at a closed posting, and the route they could still apply
    through is folded away underneath it.
    ...
    afterwards - and the world does move: a collector that detects closures
    pushes `delist` for rows like these every day.
    ...
    that found it: somebody reopens a wrongly-closed posting with `delist
    --back`, and the group stayed flipped forever, because nothing was watching
    for the reverse. Re-judging with the same function find() uses means there is ONE
