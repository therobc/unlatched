...
def _dpapi(data: bytes, *, encrypt: bool) -> bytes | None:
    ...
    # tooling; naming the app there beats an unlabeled secret. No entropy
    # argument: it would have to be stored beside the blob to be usable,
    # which adds nothing an attacker with file access does not already have.
    ok = fn(ctypes.byref(source), "Unlatched USAJOBS API key", None,
