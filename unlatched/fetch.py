...
# below - verified by construction in the _rng.uniform(1.0, 1.0 +
# JITTER_FRACTION) call below, whose lower bound is always 1.0 - jitter
# that could shorten the gap would be a way of asking faster.
JITTER_FRACTION = 0.5
...
# collectors do not agree on how busy that is - verified at
# sources/smartrecruiters.py:40 (DETAIL_CAP = 60),
# sources/bamboohr.py:28 (MAX_DETAIL_FETCHES = 150), and
# sources/workday.py:73 and sources/oracle_hcm.py:86 (both
# MAX_DETAIL = 200). So one
...
# Seeded on demand by reset_rate_limits() so a test can pin the jitter -
# verified by construction in the `if seed is not None: _rng.seed(seed)`
# guard there. Real
...
# tenant monopolising it - verified by construction, run_expired() and
# employer_expired() below are two independent functions the collect
# loop can act on differently. Without it a hang on an employer near the top of the
...
# was reached. Verified by construction: the body is therefore read in
# chunks against a wall clock, in _read_capped.
#
...
def _respect_rate_limit(host: str, delay_s: float) -> None:
    ...
    # A LONGER REST every so often - verified by construction in the
    # `count % REST_EVERY == 0` check below. Without this a long run holds continuous
...
def _note_throttled(host: str, status: int, retry_after: str | None) -> None:
    """Record that a host pushed back, and stop asking if it keeps doing
    this, verified by construction in the THROTTLE_LIMIT check below.
    ...
    # rate limiter is built expecting it. Verified by construction in the
    # time.sleep(min(pause, MAX_RETRY_AFTER_S)) call below.
...
def fetch(url: str, *, timeout: float = DEFAULT_TIMEOUT_S,
    ...
    """
    # PAST THE RUN'S CEILING, AND CHECKED FIRST - verified by construction:
    # the check two lines below runs before `allowed()` (the scheme test,
    ...
    # Believed, not measured, that nine and a half
    # hours of a real run once passed without a single
    # employer completing - no history under app/ to check that against.
    #
    # And FIRST within it because resolving a host is a network operation that
    # can hang on its own. Believed, not measured, that a System log for one
    # night carries a DNS timeout
    # at 03:57 - no history under app/ to check that against. A ceiling whose
    # opening move is a name lookup can be held by the
    ...
    # rest of this run - verified by construction, checked BEFORE robots two
    # lines below, because the cheapest request is
    # the one never made.
    ...
    # A per-call opener, not the module-level default one - verified by
    # construction: the opener below is built inside fetch()'s own body.
    # `allowed` closes
...
def reset_rate_limits(seed: int | None = None) -> None:
    ...
    # does - verified by construction two lines below (_run_deadline and
    # _employer_deadline both reset to None here). A test that set one
    # would otherwise poison every later test in
    # the same process.
