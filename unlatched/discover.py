...
checked:
  ...
  2. Fetch a short, fixed list of conventional careers paths on the domains
     ...
     anything on it is trusted (`page_confirms_company``page_confirms_company`).
     A domain that merely exists is not the employer's site.
...
# collide with unrelated registered domains constantly - probing "Lineage
# Logistics" resolves lineagelogistics.com AND lineage.com AND lineage.io,
# and every extra "verified" domain multiplies the fetch budget by eleven.
...
ATS_FINGERPRINT = [
    # Four shapes, all seen live. The original pattern covered only the first
    # and third, which silently cost employers we can fully collect from:
    # Elation Health publishes ".../embed/job_board/js?for=elationhealth" and
    # PerfectServe the bare "boards.greenhouse.io/perfectserve", and both were
    # recorded as having no board at all.
    #
    ...
    # ("{tenant}.fa.{pod}.oraclecloud.com", e.g. "eluq.fa.us2.oraclecloud.com"),
    # linking to a candidate experience site keyed by an opaque site id that
    # is NOT always "CX_<number>" - Marriott's is the bare slug "CX", Waste
    # Management's is the custom name "WMCareers". Capturing whatever site
    # id the company's own page actually links to is why the second group
    # exists, rather than assuming a numbering scheme.
...
def page_confirms_company(html: str, company: str) -> bool:
    ...
    Generic words are excluded so a name like "American Specialty Health"
    has to match on something better than "american" or "health".
    """
    if not html:
        return False
    low = html.lower()
    ...
    if not toks:
        ...
        # words are all generic - "PNC Financial Services", "GXO Logistics",
        # "UPS Supply Chain Solutions". Every 4+ token is generic, so the
        # rule above finds nothing, and squashing the WHOLE name looks for
        # "pncfinancialservices", a string that appears on no page anywhere.
        # PNC's own careers page (157KB, saying "PNC" throughout) was
        # rejected exactly this way and the employer recorded as dead.
        #
        ...
        # below, which is what already carries IBM, SAP and ABB.
