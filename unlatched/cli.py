"""cli.py - The command surface. Constraint 5 in the project spec: anything
the desktop UI can do, this exposes too, and vice versa - the desktop app
drives long-running work by spawning this CLI and streaming its output.
...
def cmd_discover(args: argparse.Namespace) -> int:
    ...
    for name in names:
        ...
        if res["ats"]:
            first = res["ats"][0]
            ats_name = first["provider"]
            ats_ref = "|".join(first["parts"]) if ats_name in ("workday", "oracle_hcm") else \
                (first["parts"][0] if first["parts"] else "")
...
def _collect(args: argparse.Namespace) -> int:
    ...
    for company in companies:
        ...
        try:
            jobs = collector.collect(ats_ref, fetcher=fetch_mod.fetch)
    ...
    # afterwards found an Ashby row duplicating an imported LinkedIn row on the
...
def cmd_recheck(args: argparse.Namespace) -> int:
    ...
    if args.json:
        _print_json(result)
        return 0
    print(f"checked {result['checked'] + len(result['unreadable'])} added links")
    for key in result["gone"]:
        print(f"  taken down: {key}")
    for key in result["back"]:
        print(f"  back up   : {key}")
...
def cmd_forget_company(args: argparse.Namespace) -> int:
    ...
    feeds a bad name - the collector author's tracker read LinkedIn's verification badge as
...
def cmd_retire(args: argparse.Namespace) -> int:
    ...
    selection at a time. The collector author asked whether `forget-company` was the right
    instrument for 26 rows from blacklisted reposters (2026-08-11). It is not:
    that verb refuses while any job points at the company, and the company
    record is not the problem - the jobs are.

    WHY NOT status=pass, the other thing she offered. 'pass' means the PERSON
    ...
    subject. Recording them as passed would tell the first user they had rejected jobs he
    never saw, and would corrupt the only record of what he actually looked at.
...
def cmd_delist(args: argparse.Namespace) -> int:
    ...
    same pages again would be a second automated reader for no new information -
    so it is told the answer instead of going to look (the collector author, 2026-08-08, for
    the LinkedIn rows she collects).
...
def group_new_duplicates(args: argparse.Namespace) -> None:
    ...
    or an import passing --dedupe. The first user referred to the scheduled run as "the
    daily refresh and dedup" (2026-08-09) - which is what it should be, and was
    not. A board that regroups only when asked shows the same job twice every
    morning, which is the exact thing an earlier change exists to stop.
...
def cmd_brief(args: argparse.Namespace) -> int:
    ...
    An assistant running on this machine - Claude Code in an editor, say - can
    already drive every command here, read the SQLite file directly and write
    the result back with `resume attach`. What it could not do was find out
    ...
    # reason this section exists (Decided 2026-08-12: "Everything else needs to be
    # AI accessible like resumes for writing").
    #
...
def _attach_trust(args: argparse.Namespace) -> str:
    ...
    DEFAULTED TO 'mine' ON PURPOSE, per the first user (2026-08-12): everything except
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # WHICH EMPLOYERS, BY WHERE THEY CAME FROM. The first user asked for a manual menu
    # offering "all boards" and "seeded companies" as separate actions; this is
    # what makes the second one expressible. Before companies.origin existed,
    ...
    sp.add_argument("--status", default=None,
                    help="what you already did about this job (applied, "
                         "interviewed, denied...). Only SET, never cleared - "
    ...
    sp.add_argument("--show-closed", action="store_true",
                     help="include pass/denied/closed rows")
    sp.add_argument("--json", action="store_true")
...
def line_buffer_output() -> None:
    ...
    The first user, watching a run: "There should be indication that it is collecting."
