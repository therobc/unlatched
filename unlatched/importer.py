...
def check_collector_id(collector: str) -> str:
    ...
    CASE IS NORMALISED, NOT REFUSED. The config is hand-written and "LinkedIn"
...
# Markers that must never arrive AS a destination. The collector author's contract says
# 'easy-apply' and 'closed' are classifications, not URLs, and she asked to be
# told if one ever appears in the apply_url field. That audit needs a check,
...
def marker_in_destination(row: dict[str, Any]) -> str:
    ...
    The collector author asked to hear about this specifically, and it is worth checking
    rather than trusting, because the failure is silent in the worst way: a
    literal 'easy-apply' sitting in the destination column is a NON-EMPTY value,
    so it would pass every "does this row have a destination" test - and then
    every easy-apply row in the batch would carry the SAME string and dedupe
    would fold them all into one job.
    """
...
def namespaced_key(collector: str, given: str) -> str:
    ...
    linkedin collector carrying "indeed:123" does not get to write into
    indeed's rows; it becomes "linkedin:indeed:123", which is unambiguous and
    still cannot collide with anything of indeed's.
    """
...
def _key_for(row: dict[str, Any], collector: str = SOURCE_NAME) -> str:
    ...
    THE COLLISION THIS PREVENTS. Two collectors both reading the same LinkedIn
    posting derive the same stable id from the same URL, so without a namespace
    ...
    those rows matching. rekey.py has since corrected them. The requirement was "get it
    right. I don't care if you have to move the pipeline" - so the exception
    now does the opposite of what it was for: a sender still writing `manual:`
    keys would create a second copy of every row beside the corrected one.
...
def import_all(con: sqlite3.Connection, cfg: dict[str, Any],
    ...
    # the defect the collector author asked to be told about, and it was previously
    # indistinguishable from an ordinary easy-apply row.
    if markers:
