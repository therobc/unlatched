...
# What a collector may call itself. It becomes a key prefix and a
# jobs.source value, so it has to be a plain token - verified by
# construction in the regex below (a-z0-9, underscore, hyphen only): not a
# path, not a name with a colon in it that would fake a second namespace.
...
# rejected: the collector on the other side has its own richer schema
# (rank, fit_score, drop_reason, clearance, travel...) and none of it is
# load-bearing here, so an import must not fail because the sender knows
# more than we store. Verified by construction: the unknown-column check
# below reports rather than refuses.
...
# Markers that must never arrive AS a destination. The handoff contract
# says 'easy-apply' and 'closed' are classifications, not URLs, and one
# appearing in the apply_url field is a defect worth reporting. Verified
# by construction: marker_in_destination() below checks apply_url
...
def import_row(con: sqlite3.Connection, cfg: dict[str, Any], row: dict[str, Any],
    ...
    fields.update({
        ...
        # THE COLLECTOR, not the constant "imported": verified by construction
        # in the line below (`"source": collector`). Believed, not measured,
        # that every row once collapsed to one label - there is no history
        # under app/ to check that against.
        "source": collector,
        "title": title,
        "location": location,
        "url": url,
        ...
        # job: verified by construction, `qualified` is hardcoded to 1 in the
        # dict below rather than computed from the title filter or the salary
    ...
    # importing at all. Without it the app holds the posting and does not
    # know the person approached that employer - so it cannot stop them
    # applying twice, which is the one thing the record exists to prevent.
    #
    # Only ever SET, never cleared: verified by construction two lines
    # below (`if recorded: ... if existing is None: set_status(...)`) - a
    # missing or empty status on the row never reaches set_status at all.
...
# The columns a template hands somebody, in the order they make sense
# to fill. NOT the same as FIELDS: `key` deliberately sits near the
# end, before the two CSV-only trailers `closed` and `generated_at`,
# because most collectors should not set it - verified by construction
# in the tuple below (key is second-to-last, not literally last).
...
def check_rows(path: Path) -> dict[str, Any]:
    ...
    for index, row in numbered:
        ...
        if unknown:
            ...
            # ignored by design. Verified by construction: the line above computes
            # `unknown` and this only calls note(), never raises or drops the row.
            # It is reported because a MISSPELLED one looks exactly like an extra
            # one, and that is the mistake this catches.
            note(index, "ignored column(s): " + ", ".join(unknown))

    if fmt == "json" and read_version(path) != CONTRACT_VERSION:
        # REPORTED, NOT REFUSED. Verified by construction: this appends to
        # `problems` rather than raising, and the function goes on to read and
        # return the file's rows regardless. A file that omits the field parses
        # identically to one that declares it, so without this the field would
...
def _text_of(path: Path) -> str:
    ...
    # utf-8-sig, not utf-8. By definition, Excel's BOM (U+FEFF) lands on
    # the FIRST HEADER NAME, so `url` arrives as `﻿url` and the column
    # silently is not there - which for a spreadsheet-first format is the
    # single likeliest way a correct file reads as empty.
...
def _csv_rows(text: str) -> list[dict[str, Any]]:
    ...
    for raw in reader:
        ...
        for name, value in zip(names, raw.values(), strict=False):
            ...
            # produces a value for every column of every row, and an empty string
            # in `key` would otherwise read as a key the sender chose. Verified by
            # construction: the `if name and value is not None and
            # str(value).strip()` guard below is what drops it.
...
def read_generated_at(path: Path) -> str:
    ...
    try:
        ...
        return ""
    if not looks_like_json(text):
        # A COLUMN, because CSV has nowhere else to put it. Verified by
        # construction: the generator below is `next(... for r in rows if
        # r.get("generated_at")...)`, so a spreadsheet repeating the value
        # down the column and a script filling only the first row both land
        # here.
        try:
            rows = _csv_rows(text)
...
def handoff_age_hours(generated_at: str, now: datetime) -> float | None:
    """Hours since the sender wrote the file (verified by construction in
    the two early-return branches below): None for an empty or
    unparseable stamp.
...
def import_all(con: sqlite3.Connection, cfg: dict[str, Any],
    ...
    for index, row in enumerate(rows):
        ...
        if marker:
            ...
            # programs. Verified by construction two lines below: `usable`
            # rebuilds the row with apply_url cleared, so the bad value
        ...
        # Rebound rather than mutated: verified by construction immediately
        # below - `dict(row, apply_url=...)` builds a new dict rather than
        # assigning into `row`, so the caller's own dict is left untouched
        # while the bad destination can never become a dedupe key.
    ...
    # Both of these are the sender's to fix, so they are REPORTED rather
    # than buried: verified by construction, `markers` and `unknown` are
    # both accumulated above and surfaced in `result` below rather than
    # raised. Believed, not measured, that a row with neither a
    # destination nor a classification was previously indistinguishable
    # from an ordinary easy-apply row - there is no history under app/ to
    # check that against.
    if markers:
