...
# Query streams that stopped at MAX_PAGES_PER_QUERY with more still available.
#
# MAX_COLLECTED CANNOT CATCH THIS, which is why the list exists. Every board
# collector reports its ceiling by the CLI comparing what came back against
# MAX_COLLECTED - but this collector de-duplicates across streams into a dict,
# so twelve streams of 500 can return far fewer than 6,000 unique postings and
# the comparison never fires. Measured: a query advertising 900 matches
# returned 500 and 400 were never seen, with `collected >= MAX_COLLECTED`
# False and nothing anywhere saying so.
#
# A silent truncation is the failure this package has been bitten by three
# times already - the board fetch cap, robots.txt on board APIs, and a
# rejected key reading as "no federal jobs matched". The same answer applies:
# say it out loud. Module-level for the same reason fetch.stopped_hosts is -
# a run summary needs to report something the return value has no room for.
_truncated: list[str] = []


def truncated_queries() -> list[str]:
    """Query streams the last run cut short, as readable lines.

    Empty after a run that read everything it was offered. Read by cli.py to
    put a note on the summary, the same way a board's ceiling is reported.
    """
    return list(_truncated)
...
def _description(descriptor: dict[str, Any]) -> str:
    ...
    # 50 carried a clean PositionSensitivity but SecurityClearance "Secret".
    ...
    if clearance.lower() == AMBIGUOUS_CLEARANCE:
        ...
        # PositionSensitivity, so reading it as "requires a clearance"
        ...
        # PositionSensitivity below is the field that actually decides.
            ...
            if expected is not None and seen_this_query < expected:
                what = " ".join(
                    p for p in (f"{term!r}" if term else "",
                                f"in {location!r}" if location else "") if p)
                _truncated.append(
                    f"{what or 'the unfiltered search'}: read {seen_this_query} "
                    f"of {expected} - narrow search.title_include or "
                    f"search.locations to see the rest")
