...
def _chat(base_url: str, api_key: str | None, model: str | None,
    ...
    # OSError as well as URLError, matching `check` below.
    #
    # MEASURED against a local server that completes the handshake and then
    # says nothing - a model that accepted the request and is still thinking:
    #
    #     raised: builtins.TimeoutError: timed out
    #       is a URLError?     False
    #
    # So a READ timeout is NOT a URLError and was escaping as a traceback
    # instead of the message the CLI prints. It is also the likeliest failure
    # of the two here, because this endpoint is usually a model on the same
    # machine, where connecting always succeeds and thinking is what takes
    # time.
