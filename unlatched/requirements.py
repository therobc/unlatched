...
class RequirementsInfo(TypedDict):
    """Every field is None/empty/False (by construction - see the return
    statements throughout this module) when the posting never raised the
...
# Longest-first so "seven" cannot win the alternation before "seventeen"
# gets a chance to be tried (Python's re backtracks either way, but
# this also keeps the pattern legible about intent). Verified by
# construction: see the sort() call immediately below.
...
YEARS_RE = re.compile(
    ...
    # the "(" itself when it is preceded by whitespace. Verified by
    # construction (see the pattern below) and tested by
    # test_parenthesized_number_is_captured_with_its_parens and
    # test_spelled_out_number_immediately_followed_by_its_digit.
...
# The EMPLOYER's own age, from the boilerplate opening paragraph. Tested
# by test_company_age_is_not_a_years_requirement ("Founded in 2001...
# in business for 25 years" -> None) and by
# test_negative_context_does_not_bleed_across_bullets.
#
# Checked against the characters IMMEDIATELY around the match rather than
# the wide YEARS_NEGATIVE window, for the reason YEARS_AGE_TRAILING
# documents: a window reaching 40 characters either way swallows the NEXT,
# unrelated years figure - verified by construction from the pad values
# below (40 here vs 90 for YEARS_POSITIVE_CONTEXT).
COMPANY_AGE_BEFORE = re.compile(
    r"for\s+(?:over|more\s+than|nearly|almost)\s*$", re.IGNORECASE)
# ", Northwind has ..." - a subject that is not the reader. Case-SENSITIVE
...
# The CANDIDATE's own minimum age ("must be at least 21 years of age") -
# a driver/security/alcohol-adjacent posting staple that reads identically
# to an experience figure ("at least N years") right up to this trailing
# phrase. Tested by test_minimum_age_is_not_a_years_requirement and
# test_minimum_age_does_not_swallow_a_nearby_experience_requirement, which
# covers exactly the "21 years of age. 5+ years of experience preferred"
# case this regex exists for.
...
# common "Bachelor's required, Master's preferred" pattern) reports the
# one that actually gates the role rather than whichever appears first in
# text. Verified by construction: the list below runs doctorate -> master
# -> bachelor -> associate -> high_school, and education_required()'s
# required_only[-1] fallback (below) depends on that order.
...
def _education_signal(text: str, m: re.Match[str]) -> tuple[bool, bool]:
    ...
    if preferred and required:
        ...
        #
        # Decided by PROXIMITY to the degree itself instead. Verified by reading
        # the `degree_at`/`abs()` comparison two lines below.
...
def education_required(text: str) -> tuple[str | None, bool | None, str]:
    ...
    if len(found) > 1:
        ...
        if required_only:
            ...
            # lowest (most permissive) genuine floor. Verified by construction:
            # EDUCATION_LEVELS starts "none, high_school, ..." while _EDU_PATTERNS
            # starts doctorate, master, bachelor, ....
...
# Structured postings often carry a "Supervisory Responsibilities:"
# heading, answered with anything from a bare "N/A" to a full sentence
# ("This role is an individual contributor with no direct reports or
# supervisory authority") for individual-contributor roles - believed,
# not measured, that this appears on EVERY structured posting rather
# than just often. Tested by
# test_supervisory_heading_answered_na_is_not_supervisory and
# test_supervisory_heading_answered_with_a_negating_sentence.
...
def supervises_required(text: str) -> tuple[bool | None, str]:
    """True when the posting states supervisory scope; tested by
    test_supervises_absent_is_none_never_false, None (never False)
...
CLEARANCE = re.compile(
    ...
    # Investigation"). Observed in the 400-posting USAJOBS sample documented
    # in tests/test_federal_vetting.py (VOCABULARY, e.g. "Position
    # Sensitivity: Moderate Risk (MR)").
    r"public trust(?:\s*(?:clearance|position))?|"
    # DOE's own ladder, which its national laboratories and production sites
    # post against. Tested against the "Q Access Authorization" case in
    # tests/test_federal_vetting.py's VOCABULARY (observed in a 400-posting
    # USAJOBS sample).
    # Agencies sometimes paste an en dash where a hyphen belongs
    # ("Q - Nonsensitive"), so accept either. Verified by construction: the
    # pattern below is written as the \u2013 escape, not a literal
    # character, so this file stays ASCII.
    ...
    # field and means the OPPOSITE - by definition a suffix pattern would
    # match both and invert one of them. Tested by
    # test_live_usajobs_vocabulary_classifies_correctly against
    # "Non-sensitive (NS)/Low Risk" (not a clearance) and
    # "Critical-Sensitive (CS)/High Risk" (a clearance).
...
# "Security Clearance: Not Required" is observed as the single most
# common clearance STATEMENT in the 400-posting USAJOBS sample
# documented in tests/test_federal_vetting.py, and reading it as
# "requires a clearance" would invert the filter - throwing away
# exactly the postings a clearance-free candidate wants. Checked in a
# window around the match, not over the whole posting, so an unrelated
# "not required" elsewhere in a long description cannot cancel a real
# requirement.
...
# "Security Clearance: Secret" matches on the label, so the evidence
# would read "Security Clearance" and tell the reader nothing they did
# not already know. When the match is a label followed by its value,
# the value is what belongs in the evidence - verified by construction
# in `_with_value` below.
...
def summary(reqs: RequirementsInfo) -> str:
    ...
    if level:
        # The extractor names levels with underscores ("high_school").
        # Normalised on the way in, by construction (the .replace("_", " ")
        # immediately below), so a new level added upstream reads as words
        # rather than code.
...
def profile_is_configured(profile: dict[str, Any]) -> bool:
    ...
    clearance_ok AND public_trust_ok ARE DELIBERATELY NOT CONSULTED, and
    their absence is not an oversight: this asks "could compare() produce a
    verdict", and those two are precisely the fields compare() never judges -
    see the module docstring. They are acted on in screen.py instead, which
    drops a posting outright on them. A profile carrying only those two would
    make this return True and then produce an empty comparison.
    """
    p = profile or {}
    return bool(
...
def _known_list(value: Any) -> list[str] | None:
    """Verified at config.py lines 236 and 240: an empty list from the
    "profile" block's licenses/willing_shifts cannot be told apart
...
def compare(reqs: RequirementsInfo, profile: dict[str, Any]) -> CompareResult:
    ...
    if (level is not None and have_level is not None
        ...
        if have_rank >= req_rank:
            ...
            # someone with years of the work and no matching diploma. Verified
            # by construction: this stretch fires only when
            # education_equivalent_ok is True, set from EDU_EQUIVALENT matches.
            stretches.append(
...
def summarize(reqs: RequirementsInfo) -> str:
    """One compact line for a listing view, verified at cli.py:897 - that
    is `unlatched show`'s own call, so a full requirements breakdown
    does not have to be printed just to tell
