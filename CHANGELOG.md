...
- **Links you add by hand fill themselves in, without turning anything on
  first.** The app could already read a posting you pasted and fill in its
  title, employer and description, and it could already tell you whether one
  had been taken down - both were switched off by default, so neither
  happened. Adding a link gave you an empty form to type into, and pressing
  re-check read nothing at all.

  Both are on now. Nothing reads a posting on a schedule: the app opens a link
  when you add it, and when you press the button, and at no other time.

- **Add a job by link is on the Dashboard too**, not only on the jobs list.

- **The reason an add failed is the reason you are shown.** When adding a job
  failed, the app displayed whatever the engine happened to print last - which
  was usually a line about duplicate grouping having succeeded, so a job that
  was refused for having no title said something about postings being still
  open instead.

- **Searching is instant, however much you are holding.** Every search read
  each job's full description from start to finish, so the more you collected
  the slower typing in the search box felt. The app now keeps an index of the
  words it has seen, and consults that instead. On this profile, a search that
  took 30 to 44 milliseconds now takes under one.

  The index is built once, takes about half a second, and adds roughly three
  megabytes. Nothing about how you search changes, and if the index is ever
  missing the app falls back to reading the descriptions as it used to.
