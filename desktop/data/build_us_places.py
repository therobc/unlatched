...
it is the government's own, so a typed location matches what employers write.
    ...
    python build_us_places.py            # downloads the current year
...
AND census-designated places, which is what makes Powell, TN and Seymour, TN
- Knoxville-area communities that are not Knoxville, and that employers do
write in postings - typeable at all. About 30,000 of them.
...
def main(argv: list[str]) -> int:
    ...
    # Largest land area wins a duplicate name, so "Knoxville, TN" is the
    # Knoxville the file keeps at 98.7 square miles rather than the 0.3 of
    # Knoxville, GA.
    ...
    # Powell, TN (a CDP, and exactly the kind of place this file exists for)
    # under every incorporated Powell in the country. Area is a proxy, not a
    # truth: it gets Knoxville TN, Portland OR and Chicago IL to the top of
    # their names, which is what the ranking is for.
    ...
    for probe in ("Powell, TN", "Seymour, TN", "Knoxville, TN", "Farragut, TN",
                  "Oak Ridge, TN", "Maryville, TN", "Alcoa, TN", "Boise, ID"):
    ...
    for typed, expected in (("knoxville", "Knoxville, TN"),
                            ("powell", "Powell, TN"),
