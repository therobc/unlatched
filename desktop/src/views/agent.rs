...
//!
//! The question this screen answers - if somebody tells an agent there is an
...
const SETUP_COMMANDS: [(&str, &str); 4] = [
    ...
    (
        "unlatched discover --name \"Nordstrom\"",
    ...
    card(ui, "THERE IS NO API, AND THAT IS THE POINT", |ui| {
        ui.label(
            "Nothing listens on a port and there is no account to create. This app \
             ...
             assistant already running here - one in your editor that can use a \
             terminal - works with it by running commands and reading what comes \
             back.",
        );
        ui.add_space(8.0);
