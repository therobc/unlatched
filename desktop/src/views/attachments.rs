...
// to add something.
//
// DOWNLOAD-ONLY, WITHOUT EXCEPTION (Decided 2026-08-13: "Build attachments as
// download only"). Nothing here opens a file - not an image, not plain text -
// so the app carries no decoder and no parser for any attachment format, and
// there is no code path where a file somebody else wrote is interpreted. The
// hover says "Download to view", or "Unsupported file type, download to view"
// for something it cannot even name. Both are his words.
//
...
///
/// the first user asked for this by name. Somebody who does not know the shortcut cannot
/// use the feature at all, and the confirmation screen is the artefact people
/// most often wish they had kept.
...
pub fn show(ui: &mut egui::Ui, files: &Files<'_>) -> Action {
    ...
    ui.weak(SNIP_HINT);
    // WHERE DOWNLOADS GO, ON SCREEN. The first user asked for the choice to be remembered
    // per profile so several people on one machine can keep their files apart;
...
mod tests {
    ...
    fn the_paste_control_teaches_the_shortcut() {
        // the first user asked for this string specifically. Without the shortcut, a
        // person who does not already know it cannot use the feature at all.
