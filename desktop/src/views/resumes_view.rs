...
/// Roles, matching the engine's resumes.py. Original is what the person
/// started with; optimized is the version edited after the app's advice, and
/// it is the one screening reads when it exists.
...
fn versions_list(app: &mut UnlatchedApp, ui: &mut egui::Ui) {
    ...
    let dir = app.active_home.join("resumes");
    let mut files: Vec<(String, String)> = match std::fs::read_dir(&dir) {
        Ok(entries) => entries
            .filter_map(|e| e.ok())
            ...
            .filter_map(|e| {
                ...
                if role == ORIGINAL || role == OPTIMIZED {
                    Some((role, name))
                } else {
                    None
                }
    ...
    // The stamp is written into the name as YYYYMMDDTHHMMSS, so a plain
    // reverse sort is newest-first without parsing a date.
    ...
    // Screening prefers the newest optimized copy, then the newest original -
    // the same order resumes.active_path uses.
    let active = files
        .iter()
