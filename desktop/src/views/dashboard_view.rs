...
///
/// Accepted Offer is its own rung by construction - status::FLOW gives it
/// rung 3 between Offer at 2 and Hired at 4 - because it is a stage a
/// person occupies rather than passes through. How long they occupy it is
/// believed, not measured. Folding it into either neighbour would report a
/// job they have not started, or lose the fact that they said yes.
...
/// by construction, since neither function holds a table of its own any more,
...
pub fn show(app: &mut UnlatchedApp, ui: &mut egui::Ui) {
    ...
    // Set inside the header closure and acted on after it. The menu borrows
    // the app to read its collector list while it draws, so starting a
    // process in there needs a second, mutable borrow - enforced by the type
    ...
    ui.horizontal(|ui| {
        ...
        ui.add_space(10.0);
        // ADDING FIRST, then the one that refreshes: the order triage.rs uses,
        // where it is recorded as decided 2026-08-07.
        if crate::access::tag(
        ...
        .on_hover_text(
            "Paste the link to a posting you found yourself. It joins your \
        ...
        .clicked()
        {
            ...
            // refresh - and if that anchor was already satisfied, until the
            // following day. Reading a local file is not going out to the boards,
            // so this keeps the promise the hover text makes.
            //
            // Harmless when there is nothing new: verified in cli.cmd_ingest, which
            // reports "nothing new" and writes no rows when the file's fingerprint
            // is unchanged. Skipped while another command is running, which
            // start_process reports in the log.
            if app.busy() {
                // SAID OUT LOUD. start_process refuses while another command runs and
                // reports it only to the log, so pressing this during a collect would
        ...
        // TAKING IN ONE COLLECTOR, from the screen that says one has delivered.
        // Refresh above pulls every collector at once, which is the right
        // default and the wrong tool when somebody can see on this very screen
        // that one of three has a new file waiting - the age and the taken-in
        // line are both here already, and acting on what they say meant going
        // to the Companies page to find the menu.
        //
        // THE WHOLE MENU, not the handoff half of it. Only the handoff submenu
        // was here first, which left this screen able to take in a file another
        // program wrote and unable to read a job board - on the one screen that
        // says how stale the boards are.
        //
        // ONE DEFINITION, shared with the Companies page rather than copied.
        // The partial copy IS the argument: see views::collect_menu.
        //
        // NO EMPTINESS GATE HERE, unlike the handoff-only version this replaced.
        // That gate existed because a menu whose sole content could read "No
        // collectors are set up" is a control that never does anything. This
        // menu always has boards to offer - by construction, since the board
        // entries do not depend on any collector being configured - and its
        // handoff submenu carries the emptiness rule inside it, in
        // collectors_menu::offer_from.
        ui.add_space(6.0);
        ...
        ui.columns(2, |cols| {
            cols[0].push_id("source_card", |ui| {
                card(ui, "WHERE YOUR MATCHES COME FROM", |ui| {
                    // A COUNT ALONE CANNOT SAY A FEED HAS DIED. Observed: this panel
                    // read `imported 410` for the ten days that collector was handing
                    // over nothing - a true number about a dead source. The age is what
                    // makes that visible.
...
fn stat_cards(app: &mut UnlatchedApp, ui: &mut egui::Ui, stats: &DashboardStats) {
    ...
    // pulled" is a different kind of event from a listing you never acted
    // on expiring. That it is worth interrupting somebody for is believed,
    // not measured - what is certain is that the two events are not the
    // same one, and a card would have counted them together.
...
fn stat(
    ...
    let response = egui::Frame::none()
        ...
        .show(ui, |ui| {
            ui.vertical(|ui| {
                // THE NAME AND THE COUNT GO ON THIS LABEL, a real laid-out widget,
                // NOT on the interact() overlay below.
                //
                // Observed when they were on the overlay: it hung the accessibility
                // tree outright - the first UIA walk of every run blocked, and three
                // harness runs had to be killed. An interact() region is not a widget
                // egui has placed, it is a hit area over one already drawn, so
                // attaching widget_info to nine of them per frame produced a tree the
                // provider could not answer for.
                //
                // Diagnosed by breadcrumb rather than by reading: the harness records
                // each step to disk, and it stopped at exactly "walking the
...
/// A ring with the total in the middle, drawn as a fan of triangles per
/// slice and then punched out with a centre disc in the panel colour.
///
/// The centre figure is the point: the total sits inside the ring so the
/// eye lands on "how many" before it lands on the proportions. That this
/// is the right order for this number is believed, not measured.
fn doughnut(ui: &mut egui::Ui, slices: &[(String, i64, egui::Color32)], total: i64) {
    const SIZE: f32 = 150.0;
    const SEGMENTS_PER_TURN: usize = 96;

    // The one painted figure on this screen with no text equivalent beside
    // it. Verified against the others: the legend dots, the source bars and
    // the coverage strip each sit next to a real label carrying the same
    // number, so naming them would announce every figure twice. The ring's
    // centre total is only ever painted, so without this it is unreadable
    // to anything but an eye.
    ...
    if total <= 0 {
        ...
        // Start at twelve o'clock rather than at three. Where a reader expects
        // a chart to begin is believed, not measured; the constant is here so
        // the choice is visible rather than buried in an offset.
    ...
    // The hole, filled with the window background rather than a fixed
    // colour - so by construction it matches whichever theme is drawing and
    // the ring reads as a ring rather than as a pie with a grey dot on it.
    ...
    painter.text(
        ...
        egui::Align2::CENTER_CENTER,
        // NAMES WHAT IT COUNTS. Observed: this said "applications" over a total
        // that included every auto-closed posting, so the ring reported 863
        // applications on a search with 53. The slices are the statuses a person
        // set themselves - applied, passed, and whatever an application became -
        // which is a decision, not an application.
        "decided",
...
/// Applied -> Interview -> Offer -> Hired, in pipeline order, with the
/// rates underneath as a sentence.
///
/// Every stage counts everyone who REACHED it, so by construction the
/// bars can only shrink. Counting only those currently sitting at a stage
/// produces a funnel whose middle is wider than its top.
fn funnel(ui: &mut egui::Ui, stats: &DashboardStats) {
    ...
    // The bar takes the colour of the status it counts, from the one
    // palette in crate::status - by construction the same source the pill
    // on the pipeline reads, so the two cannot drift into two designs that
    // happen to agree.
...
fn collector_file_status(app: &mut UnlatchedApp, ui: &mut egui::Ui) {
    ...
    // load. Until the stamps are re-read for it, "no rows for this
    // collector" is indistinguishable from "we have not looked" - observed
    // reading a day-old file as never imported. Happens once, when the
    // listing lands.
    ...
    // LEFT TO RIGHT since this moved off the header on 2026-09-05. It was
    // right-to-left to pin it against the far edge of a crowded row; on its
    // own line that would strand it across the screen from the notice
    // beside it.
    ui.horizontal(|ui| {
        for entry in live.iter() {
            // THE FILE, RE-STATTED, not the age the engine reported when this
            // profile opened. Observed: that one never moved, so a collector
            // finishing at lunchtime changed nothing on this screen.
...
fn source_rows(ui: &mut egui::Ui, bars: &[&SourceBar], max: i64) {
    ...
    for bar in bars {
        ui.horizontal(|ui| {
            ...
            // A late collector's BAR stays the ordinary colour. The rows it brought
            // are as real as any other; it is the age that is wrong, so the age is
            // what changes. Believed to read better than colouring the bar, not
...
/// COLLECTS can report at all.
///
/// Observed on a real search: one person matched a single job because
/// four of their twenty-seven employers had a board this app could read.
/// Every thin result set in this project has traced back to coverage.
