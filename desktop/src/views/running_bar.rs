...
//! which is exactly what the first user reported: "There should be indication that it is
//! collecting."
//!
    ...
    egui::TopBottomPanel::bottom("running-bar").show(ctx, |ui| {
        ...
        ui.horizontal(|ui| {
            match &running {
                ...
                None => {
                    let line = finished.clone().unwrap_or_default();
                    let colour = if line.contains("failed") { FAILED } else { DONE };
                    crate::access::tag_with_value(
