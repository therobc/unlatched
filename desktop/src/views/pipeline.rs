...
pub fn show(app: &mut UnlatchedApp, ui: &mut egui::Ui) {
    ...
    // A job with notes but no status change still has a history worth showing.
    for key in notes_by_key.keys() {
        groups.entry((*key).to_string()).or_default();
    }

    if groups.is_empty() {
        ui.label("No status history yet. Mark a job from Triage to start one.");
        return;
    }
...
/// interviewed and denied as replies, which was written before Offer and Hired
...
fn render_summary(app: &UnlatchedApp, ui: &mut egui::Ui) {
    ...
    ui.horizontal(|ui| {
        ui.label(format!("Applied: {}", reached.applied))
            .on_hover_text(
                "Jobs you recorded an application for, including ones that have \
                 since been denied. A floor: it can only count what you marked.",
            );
        ui.separator();
        ui.label(format!("Heard back: {}", reached.responded))
            .on_hover_text("Interviewed, offer, hired or denied - a reply either way.");
        ui.separator();
...
/// label is easy to miss among other coloured labels, and the reference
/// dashboard uses a badge here for the same reason.
///
...
mod tests {
    ...
    fn a_job_with_only_notes_still_produces_a_timeline() {
        // the first user asked for notes that are not tied to a status change. A person
