...
/// Room kept at the right of the table for the column-settings gear, and a
/// thin band so the table never sits flush against the window frame.
const COLUMN_SETTINGS_STRIP: f32 = 30.0;
const EDGE_BAND: f32 = 10.0;

/// Every job still worth looking at, in one list.
///
/// The rule the first user set: show everything that has NOT been taken down - and keep a
/// taken-down posting visible anyway if he applied to it, until he records the
/// outcome by marking it Denied. A job you applied to does not stop mattering
...
fn show_list(
    ...
    // an afternoon cannot turn into five requests for the same page (the first user,
    ...
    let last_collected: Option<String> = app
        .conn
        .query_row("SELECT MAX(fetched_at) FROM jobs", [], |r| r.get(0))
        .unwrap_or(None);
        ...
        if links.any() {
            ...
            if response
                .on_hover_text(if ready {
                    ...
                    "Waiting for the run in progress to finish - see the bar at \
                ...
                .clicked()
            {
                app.start_process("check added links", vec!["recheck".to_string()]);
            }
        }
    ...
    if app.selected_keys.is_empty() {
        ui.label(
            "keys: up / down move, o open, a applied, p pass, d no offer, \
             i interviewed, n note. Each status key opens the note prompt - \
    ...
    // Where the table starts, taken BEFORE it is drawn so the gear can be put
    // level with the headings afterwards.
    let table_origin = ui.cursor().min;
    let columns_width = (table_width - COLUMN_SETTINGS_STRIP - EDGE_BAND).max(600.0);
    ...
    {
        ui.scope(|ui| {
        ui.set_max_width(columns_width);
            ...
            for id in &shown {
                ...
                header.col(|ui| {
                    ...
                    let response = match spec.sort {
                        Some(column) => sortable(ui, app, column, spec.heading, &mut sort_click),
    ...
    // The gear, placed into the strip the table reserves for it rather than
    // laid out beside it. A TableBuilder inside a horizontal layout draws its
    // BODY starting where the header left the cursor - which put every row off
    // to the right of the table, in the strip, while the headings stayed put.
    // ui.put writes to an absolute rectangle and does not move the cursor, so
    // the table keeps the whole vertical layout to itself.
    let gear_rect = egui::Rect::from_min_size(
        egui::pos2(table_origin.x + columns_width + 2.0, table_origin.y),
        egui::vec2(COLUMN_SETTINGS_STRIP - 4.0, 22.0),
    );
    // A bare glyph is not a name: nothing readable, and nothing an automated
    // flow could address.
    let gear = crate::access::tag(
        ui.put(gear_rect, egui::Button::new("\u{2699}").frame(false))
            .on_hover_text("Columns: what is shown, and in what order"),
        egui::WidgetType::Button,
        "column-settings",
    );
    if gear.clicked() {
                    ...
                    let response = crate::access::tag(
                        ui.button("Mark taken down").on_hover_text(
                            "The employer pulled these postings. They leave your \
                             lists and read as No longer open. Anything you had \
...
pub fn status_note_window(app: &mut UnlatchedApp, ctx: &egui::Context) {
    ...
    egui::Window::new(format!("Mark {subject} {status_label}"))
        ...
        .show(ctx, |ui| {
            ...
            ui.add_space(10.0);
            ui.weak("Enter saves. Esc leaves the status alone.");
            ui.add_space(4.0);
    ...
    // Enter saves and Escape cancels, so keyboard triage stays keyboard
    // triage: 'a' then Enter is two keystrokes on a job that needs no comment,
    // which is most of them.
    //
    // THE NOTE FIELD IS NOT FOCUSED ON OPEN, and Enter is ignored while it is.
    // A multiline box needs Enter to mean "new line" - egui reports the key as
    // pressed even after the field consumes it, so an auto-focused box would
    // have saved the change on the first paragraph break every time. Ctrl+Enter
    // is the save for somebody who is already typing.
    ctx.input(|inp| {
        if inp.key_pressed(Key::Escape) {
            cancel = true;
        }
        if inp.key_pressed(Key::Enter) && (!typing || inp.modifiers.command) {
            save = true;
        }
    });
...
fn draw_cell(ui: &mut egui::Ui, id: columns::ColumnId, c: &CellCtx, out: &mut RowActions) {
    ...
    match id {
        ...
        C::Title => {
            ...
            let response = match &r.job.url {
                ...
                Some(url) => match fmt::safe_link(url) {
                    Some(safe) => ui.hyperlink_to(title, safe),
                    None => ui.label(title),
                },
                _ => ui.label(title),
            };
            response.on_hover_ui(|ui| {
                ...
                if let Some(desc) = &r.job.description {
        ...
        C::Match => {
            ...
            // as a clean one at a glance.
            let verdict = fmt::opt_str(&r.job.verdict);
            if verdict == "alt" {
                ui.colored_label(egui::Color32::from_rgb(200, 150, 60), "alt")
                    .on_hover_text(
                        "Matched your search but fell short somewhere: pay under \
                         the floor, a stated requirement you may not meet, or a \
                         description too thin to judge. The reason is in the \
                         panel below.",
                    );
            } else {
                ui.label(verdict);
        ...
        C::Status => {
            // A dropdown per row, the way the original web app did it. The
            // keyboard shortcuts still work for anyone who wants them, but they
            // act only on the highlighted row, and picking a status should not
            // require first selecting the right line.
        ...
        // the collector's name, not a place. The first user asked for both so they can work
        // from either (2026-08-07): the host names the board, and clicking it
        // opens the posting.
        C::FoundAt => {
            ...
            ui.hyperlink_to(fmt::link_host(url), url).on_hover_text(url);
        }
    }
}
...
fn expanded_block(
    ...
    // A HEADER BAR, not a line of text jammed against the row above. The
    // reference web app gives the open posting its own banded strip - title,
    // company, size on the left, Close set in from the right - and that band
    // is what separates "the job you are reading" from "the row you clicked"
    // and from "the next job". Without it the three run together.
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                if let Some(link) = r.job.url.as_deref().and_then(fmt::safe_link) {
                    ui.add_space(6.0);
                    ui.hyperlink_to("Open posting", link);
                }
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
    ...
    ui.indent("posting_facts", |ui| {
        ...
        if !repost_note.trim().is_empty() {
            ui.weak(&repost_note);
            // THE LINK BEN ASKED FOR. A new entry after four weeks points back
            // at the round it followed, and both stay visible - so this offers
            // the earlier posting rather than describing it.
...
/// A column header that sorts. The arrow marks the active column and its
