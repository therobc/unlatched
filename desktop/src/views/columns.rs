...
pub fn from_keys(keys: &[String]) -> Vec<ColumnId> {
    // RETAIN, NOT dedup(). Vec::dedup only collapses ADJACENT repeats, so a
    // saved order naming a column twice with anything in between kept both -
    // and the table then drew that column twice, with two rows in the gear
    // panel whose move buttons fought each other. The guard was there; it
    // just did not cover the shape a hand-edited or half-written file has.
...
mod tests {
    ...
    /// A column named twice is drawn once. Vec::dedup only collapses
