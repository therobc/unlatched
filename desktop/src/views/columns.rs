...
//
// Believed, not measured, that this table used to be rendered
// POSITIONALLY: one block declaring widths, a
// second declaring headings, a third drawing cells, kept in step only by being
// written in the same order - no history under app/ to check that
// against. Adding a column meant editing three places and
...
//
// So a column is one value that knows its own width, heading and cell -
// verified by construction in the ColumnSpec struct below. Order
...
pub struct ColumnSpec {
    pub id: ColumnId,
    /// What this column is called on disk. Deliberately NOT the heading -
    /// verified by construction, `key` and `heading` are separate fields
    /// below and from_keys/by_key resolve on `key` only: a
    ...
    /// Cannot be hidden. Title only - verified by construction, Title's
    /// spec below sets pinned: true and `visible()` always keeps a pinned
    /// column: a job list with no job names is a state a
...
pub const COLUMNS: &[ColumnSpec] = &[
    ...
    // Believed, not measured, that the cell holds a 100px
    // dropdown AND the how-long-since figure beside it and that at 110
    // the figure was
    // clipped to its first letter - no history under app/ to check that
    // against.
    ColumnSpec {
...
/// saved order predates. Verified by construction below: the first is
/// dropped by filter_map/by_key returning None, the second is appended
/// in its default place by the trailing for loop - so adding a column
/// in a later release does not require
/// everybody to reset their layout to see it.
pub fn from_keys(keys: &[String]) -> Vec<ColumnId> {
    // RETAIN, NOT dedup() - verified by construction two lines below.
    // By definition, Vec::dedup only collapses ADJACENT repeats, so a
    // saved order naming a column twice with anything in between kept both.
    // Believed, not measured, that the table then drew that column twice
    // with two rows in the gear
    // panel whose move buttons fought each other - no history under app/ to
    // check that specific incident against.
    let mut order: Vec<ColumnId> = keys.iter().filter_map(|k| by_key(k)).collect();
    let mut seen: Vec<ColumnId> = Vec::with_capacity(order.len());
    order.retain(|id| {
...
/// empty-result guard here - verified by construction: every order
/// comes from `from_keys` or
...
/// stops short of the edge and leaves the dead strip this rule exists to
/// remove.
...
pub fn settings_panel(
    ...
    // scrolling at all - verified by construction, max_height(470.0) below
    // exceeds the column count. The ScrollArea stays because a later release adding
        ...
        .show(ui, |ui| {
            for i in 0..order.len() {
                ...
                ui.horizontal(|ui| {
                    ...
                    // The tick-box column's heading is deliberately blank - verified by
                    // construction, the Select spec above sets heading: "" - so
...
mod tests {
    ...
    fn a_saved_order_from_an_older_build_gains_the_new_columns() {
        ...
        // it because their saved order does not mention it. Verified by the
        // assertion three lines below (order.len() == COLUMNS.len()).
    ...
    /// A column named twice is drawn once. By definition of the Rust
    /// stdlib, Vec::dedup only collapses
    /// ADJACENT repeats, so the original guard let a non-adjacent repeat
    /// through - and two copies of a column is a table a person cannot fix
    /// from the gear panel, because both rows answer to the same name.
    /// Tested by this function itself, below.
    #[test]
    fn a_column_named_twice_in_a_saved_order_appears_once() {
        let order = from_keys(&[
            "title".to_string(),
            "company".to_string(),
            "title".to_string(),
        ...
        assert_eq!(
            order.iter().filter(|id| **id == ColumnId::Title).count(),
            1,
            "Title was drawn twice"
        );
        assert_eq!(order.len(), COLUMNS.len(), "and nothing else was lost");
        // The FIRST mention decides the position, which is the one the person
        // last dragged it to.
        assert_eq!(order[0], ColumnId::Title);
        assert_eq!(order[1], ColumnId::Company);
    }
