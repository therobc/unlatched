...
// resumes::active_path - verified at app.rs:1711-1712, the same call that
// feeds screening. Believed, not measured, that this screen used to read
// config.resume_path instead - there is no history under app/ to check
// that against.
//
...
///
/// By definition a `Vec<String>` is one heap allocation per pushed token,
/// where borrowing a slice of an already-lowercased String is not.
/// Believed, not measured, that the previous version built the former and
/// that it froze at a corpus of exactly 82 to 6,347 postings - there is
/// no history under app/ to check those figures against.
...
fn matches_inflected(token: &str, base: &str) -> bool {
    // Suffix arithmetic, NOT format!. Believed, not measured, that the
    // previous version built five Strings per comparison and hung the UI
    // thread on a full corpus - no history under app/ to check that
    // against. This is the innermost call in the whole report, run once
    // per token per skill; the current cost is tested by
    // a_full_corpus_report_is_fast_enough_for_the_ui_thread.
    if token == base {
        return true;
    }
...
/// to an interior word. Tested by
/// multi_word_terms_match_only_as_a_contiguous_run.
...
/// real, badly-matched resume looks like. Tested by
/// an_unreadable_resume_is_named_rather_than_read_as_a_bad_match. Naming
/// the reason costs a line and removes the ambiguity.
///
...
/// tested by the_engine_reads_every_format_this_screen_promises_it_reads.
...
/// in demand keep `skills`' own order, by definition of Rust's
/// Vec::sort_by (a stable sort) - verified by construction in the
/// report.sort_by call below.
pub fn compute_report(
    ...
    //
    // Believed, not measured, that scanning every document for every skill
    // on a real profile is ~200 million window comparisons and froze the
    // UI thread - no history under app/ to check that figure against. The
    // index turns the common case into a handful of hash
    // lookups, and it is EXACT rather than an approximation, verified by
    // construction and by the_index_agrees_with_an_exhaustive_scan:
    ...
    for (doc, tokens) in corpus_tokens.iter().enumerate() {
        ...
        for token in tokens {
            ...
            // verified by construction in the `postings.last() != Some(&doc)`
...
pub fn show(app: &mut UnlatchedApp, ui: &mut egui::Ui) {
    ...
    // SAID BEFORE THE LISTS, because it changes what they mean - verified
    // by construction, the gap message below is rendered before the gaps
    // and covered lists are even built. With no
...
mod tests {
    ...
    fn inflections_match_the_python_matcher() {
        ...
        // tested by the assertion on the line directly below.
    ...
    /// in GAPS, COVERED empty, and no explanation. Tested by this function
    /// itself, below. Somebody acting on that
    ...
    fn an_unreadable_resume_is_named_rather_than_read_as_a_bad_match() {
        ...
        // the assertion two lines below (ResumeGap::NotHere).
        ...
        // better than reporting a clean run against nothing. Verified by the
        // assertion two lines below (ResumeGap::NotAnywhere).
    ...
    /// not become wallpaper that is always on screen. Tested by this
    ...
    /// that no longer exists. Tested by this function itself, below, which
    /// reads resumes.py's own source to check.
    #[test]
    ...
    /// The report at the scale that froze the app. Believed, not measured,
    /// that a real profile ever reached exactly ~6,300 postings against
    /// ~97 configured skills, or that Windows marked the window Not
    /// Responding - no history under app/ to check those figures against.
    /// The current guarantee is tested by this function itself, below
    /// (elapsed under 3 seconds at this scale).
    #[test]
