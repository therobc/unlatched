// config.json model, shared with the command-line side of the app.
//
// Every field is optional on disk: `#[serde(default)]` is used
// throughout, so by construction a missing key falls back to the
// default rather than failing the load. A profile written by an older
// build therefore opens without migration.
...
/// Employment types a search can accept, mirroring employment.KINDS in
/// the Python engine.
///
/// A posting of an unaccepted type is flagged "alt" and never dropped -
/// verified in screen.py, which sets the alt verdict rather than
/// filtering the row. The person dismisses it in triage instead, because
/// a job they never saw cannot be recovered.
...
///
/// NOT STORED IN config.json, which by construction holds only which
/// ones are enabled - so this list is the single place the UI and the
/// loader both consult when a saved file is missing a name and one has
/// to be defaulted.
...
pub struct SearchConfig {
    ...
    /// Pay under `salary_floor` but at or above this is recorded as an
    /// "alt" match rather than dropped: shown and flagged, kept out of the
    /// clean count. None disables the tier, so by construction anything
    /// under the floor drops.
    pub salary_alt_floor: Option<i64>,
    pub currency: String,
    /// Places the person can work from, as "City, ST".
    ///
    /// Empty means the question was never asked, and by construction every
    /// location passes - an empty list is not a filter that matches nothing.
...
/// SHIPS ON since 2026-09-07. The workflow it serves: a person adds a
/// link and the app fills the details in; the person clicks re-check and the
/// app says whether the posting is still up. Measured that day on a live
/// profile: the setting read false, and both consumers are gated on it - `add`
/// takes page = {} so nothing fills in, and `recheck` returns checked 0 before
/// making a request. Both halves built, neither running, so the app read as
/// broken while behaving exactly as configured.
///
/// The earlier position - that "a user turned that on" differs from "the
/// author shipped it on" - was about a host whose robots.txt disallows the
/// job page. What answers it is that every read behind this setting is a
/// person's click: adding a link, or pressing re-check. `recheck` is reachable
/// from the CLI subcommand, the Collect entry and the triage button and from
/// nowhere else; neither refresh.py nor collectors.py calls it. Attended stays
/// attended.
///
/// NOT DERIVED, deliberately. `#[derive(Default)]` gives a bool `false`, which
/// matched the engine only while the engine shipped off - so the derive turned
/// a decision into a coincidence of the type, and the two halves disagreed the
/// moment one of them changed. `test_both_halves_default_added_link_reading`
/// holds them together.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq)]
#[serde(default)]
...
pub struct RefreshConfig {
    ...
    /// updated, so the Config screen carried a "Not before hour" box that
    /// wrote a key nothing reads - verified: the engine has no mention of
    /// `after_hour` anywhere. Somebody could type an hour, press Save, and
    /// change nothing. That is the same defect this file's fetch block was
    /// cleared of once already: a control that does nothing is worse than
    /// no control, because it tells someone they have addressed a concern
    /// they have not.
    pub at: Vec<String>,
...
///
/// MODELLED SO IT CAN BE EDITED ON A SCREEN. Verified before it was
/// written: nothing on the Config screen wrote a `collectors` entry and
/// this struct had no field for one, so adding a collector meant opening
/// config.json in a text editor - the last setup step in this app that
/// needed one.
///
/// `rest` IS THE IMPORTANT FIELD. `config::save` merges over what is on
/// disk, but arrays are replaced WHOLE - deliberately, because a list
/// here is a complete answer and element-wise merging would make
/// removing an entry impossible. An entry modelled with only the four
/// fields this screen shows would therefore have deleted `schedule`,
/// `we_may_refetch` and `pushes_closures` the moment anybody pressed
/// Save. Flattening the unknown keys back out keeps them, and
/// a_collector_keeps_the_fields_this_screen_does_not_model is the guard.
#[derive(Serialize, Deserialize, Clone, Debug, Default, PartialEq)]
pub struct CollectorEntry {
    /// The namespace: it becomes jobs.source and the prefix on every key
    /// the collector writes. Verified in importer.check_collector_id, whose
    /// own note gives the reason - a collector that could put a colon in
    /// its id could claim another collector's namespace.
    pub id: String,
    /// What the menu calls it. Falls back to the id when empty.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub label: String,
    /// The file this collector leaves for us: read, never written.
    /// Nothing in this app opens a collector's path for output, so that
...
pub struct Config {
    ...
    /// engine does. Observed before it was: `unlatched resume pin` wrote the
    /// key and the engine honoured it first, but this struct had no field
    /// for it, so the Resumes tab drew its "in use" marker from the
    /// automatic rule alone - a pinned copy was scored against while the
    /// screen pointed at a different file. `save` already merged unmodelled
    /// keys through, so the pin was never lost; it was simply never read on
    /// this side.
...
impl Config {
    ...
    /// Every secret field in this config, gathered once so the two mappings
    /// below cannot drift apart as fields are added - by construction they
    /// iterate this list rather than naming fields themselves.
    ///
    /// Mirrors SECRET_KEYS in the engine's config.py; both front ends have
    /// to agree on the set.
    ...
    /// Called after reading config.json, so by construction the rest of the
    /// app works with plain strings and never has to know a value was
    /// wrapped on disk.
...
/// Loads config.json, falling back to defaults if the file does not
/// exist yet.
///
/// A parse error is RETURNED rather than discarded so the UI can show
/// it, and the config handed back is still usable defaults. That holds
/// by construction: the fallback exists before the read is attempted.
/// A profile with a broken file opens, and says so.
...
pub fn save(path: &Path, cfg: &Config) -> Result<(), String> {
    ...
    // running app holding a key it cannot send - which the clone prevents
    ...
    // writing only the modelled keys DELETED them. Observed: the file still
    // looked right, and the search quietly stopped filtering by location
    // and stopped disqualifying roles needing a clearance the person cannot
    // get.
    //
    // Re-read here rather than remembered from load, so a change made by
    // the engine or an assistant between opening this screen and pressing
    // Save survives too.
...
/// `overlay` wins wherever both name the same key; objects merge key by
/// key, so by construction unknown keys nested anywhere survive.
///
/// Arrays are replaced whole, not appended: a list in this config is the
/// complete answer to a question ("these are my job titles"), and
/// merging element-wise would make removing one impossible.
...
mod tests {
    ...
    ///
    /// `merge` replaces arrays whole - verified in merge's own note and the
    /// reason given there - so a collector entry round-tripped through a
    /// struct modelling four keys would come back as four keys, and
    /// `schedule`, `we_may_refetch` and `pushes_closures` would be gone from
    /// a file the person only meant to rename something in. Silently: it
    /// still parses and the collector still runs, just on every refresh
    /// instead of at 13:00.
    #[test]
    ...
    /// The smallest entry the engine accepts is id plus path. Verified in
    /// collectors._entry: every other field is filled from
    /// collectors.DEFAULTS, so this side must not demand more than the
    /// contract does.
    #[test]
    ...
    fn a_config_that_omits_us_only_still_filters_to_the_us() {
        ...
        // And the ordinary false default is genuinely false, so what is
        // measured here is a value rather than the existence of booleans.
        assert!(!cfg.search.travel_ok);
    }

    #[test]
    fn the_search_scope_ticks_survive_a_round_trip() {
        // Both were engine-only until the Config screen modelled them, so what
        // is measured here is that a value set on that screen reaches the file.
