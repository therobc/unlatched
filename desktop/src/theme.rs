...
//! missing glyph renders as a tofu box - a defect this fleet has shipped
//! before. Where an icon would go, this draws a shape with the painter, which
...
/// a table that already has eleven columns.
...
pub fn nav_row(ui: &mut egui::Ui, label: &str, active: bool) -> egui::Response {
    ...
    // absent from the accessibility tree - the eight controls a person uses
    // most are the eight the app never announced. The value carries which
...
/// between a list of eight links and a navigation structure.
