...
// morning has not applied to most of what is on screen. Verified by
// construction: DashboardStats below orders last_collected,
// withdrawn_after_applying and waiting_on_reply ahead of the funnel and
// by_status/by_source bands.
...
// How far each status proves a job got, and which ones mean the employer
// came back, both now live in crate::status. Believed, not measured, that
// the two were ever hand-written match arms here before - there is no
// history under app/ to check that against.
...
/// interview still reached the interview. It is a FLOOR - a person who marks
/// a job straight to No Offer never recorded the interview it may have had -
/// tested by turning_an_offer_down_still_counts_as_having_had_one and
/// a_rejection_proves_an_application_and_nothing_further.
...
pub struct Reached {
    ...
    /// folded into either neighbour. Tested by
    /// accepting_an_offer_is_its_own_rung_between_offer_and_hired.
    pub accepted: i64,
    pub hired: i64,
...
pub struct DashboardStats {
    ...
    /// intersection of two other cards rather than a pile of its own. Verified
    /// by construction: `load` below sets this from
    /// db::count_module(Module::WithdrawnAfterApplying), the same module the
    /// dashboard view's link opens.
    ...
    /// Ladder counts for the funnel, read from the append-only log - verified
    /// by construction in reached() below, which queries job_status_log
    /// directly rather than the current-status column.
    pub reached: Reached,
    ...
    /// data changes - verified in dashboard_view.rs, which reads the clock
    /// itself every frame from this offset rather than caching a computed time.
...
/// DISTINCT because a person who marks the same job Applied twice has made
/// one application; the fold would collapse it anyway, but not reading the
...
/// `external_ids` are the configured collectors, from the engine's own
/// list, passed through to db::boards_last_collected below by construction
/// so the board sweep is excluded from the freshness stamp.
pub fn load(conn: &Connection, external_ids: &[String]) -> SqlResult<DashboardStats> {
    ...
    Ok(DashboardStats {
        ...
        // today - by construction, since both are db::count_module over
        // Module::WithdrawnAfterApplying rather than two queries that
        // happen to agree.
        //
...
/// The skills most often asked for by jobs worth having, that the resume
/// does not evidence. Split in Rust rather than SQL because missing_skills
/// is stored as one comma-joined string per row - verified by construction
/// in the split(", ") below, and tested by
/// gaps_are_tallied_across_rows_and_ranked.
fn top_gaps(conn: &Connection) -> SqlResult<Vec<(String, i64)>> {
    ...
    // shuffling with the hash map's iteration order. Verified by construction:
    // the sort_by key below is (count, then name).
...
mod tests {
    ...
    /// invisible on screen until the first interview. Tested by this very
    /// function, below.
    ///
    ...
    fn every_status_a_person_can_set_stays_in_the_breakdown() {
        ...
        // And one auto-closure. Tested by the assert! three lines below that
        // "closed" is not in the resulting names.
        conn.execute(
    ...
    /// because it is what a person reads: verified in dashboard_view.rs, where
    /// the card's own count comes from `stats.module_counts.get(&module.key())`.
    ...
    fn seeded() -> Connection {
        ...
        // SCHEMA_SQL is the ORIGINAL shape, by definition (see its own doc in
        // db.rs) - migrate() below adds the columns a real install has gained
        // since. Without both calls the fixture is a database no install has had,
        // and every module clause reading a migrated column fails on it.
    ...
    fn counts_come_from_the_stored_data() {
        ...
        // Nimbus has no resolved board: verified in seeded() above, where Nimbus
        // is inserted with ats = NULL, which the employers_readable query excludes.
    ...
    fn a_job_marked_straight_to_interviewed_still_counts_as_an_application() {
        // Believed, not measured, that a real pipeline screenshot first exposed
        // this - there is no history under app/ to check it against. Somebody who
        // lands an interview and marks it as such never types the word "applied",
        // and the assertions below confirm the current fold still counts it as
        // one: applied == 2, interviewed == 1.
    ...
    fn an_offer_is_a_response() {
        // Believed, not measured, that Offer and Hired ever went uncounted as a
        // response - there is no history under app/ to check it against. The
        // assertions below confirm the current fold counts both: responded == 2,
        // hired == 1, offer == 2.
        let r = fold(&[
            ("a", "applied"),
            ("a", "offer"),
    ...
    fn passing_on_a_job_or_watching_it_close_is_not_an_application() {
        ...
        // Counting them would inflate the top of the funnel with jobs that never
        // entered it. Verified by construction: the assertion below is
        // `r == Reached::default()`.
