...
// The reference this was designed against (MadsLorentzen/ai-job-search, whose
// dashboard is specified in that repo's own agent-command file) reads ONE source:
// a CSV a human fills in AFTER applying. That makes it a pipeline tracker -
// totals, a status doughnut, a funnel. It has no collector, so it has no
// notion of what arrived overnight, what was withdrawn, or which of your
// employers it cannot even read.
//
// Ours has both halves, and the discovery half comes first: a person opening
// this app on a Tuesday morning has not applied to most of what is on screen.
// So the ordering is what changed since you last looked, then what needs a
// decision from you, then how the search is doing.
...
///
/// Cumulative, so the bars can only shrink: somebody denied after an interview
/// still reached the interview. It is a FLOOR - a person who marks a job
/// straight to Denied never recorded the interview it may have had - and every
/// caller says so rather than implying a precision the data does not carry.
...
pub struct Reached {
    ...
    /// still sits between the two, and the first user asked for that gap to be visible
...
///
/// The LOG, not the current status: a job that moved Applied -> Denied has to
...
pub fn load(conn: &Connection) -> SqlResult<DashboardStats> {
    let last_collected: Option<String> =
        conn.query_row("SELECT MAX(fetched_at) FROM jobs", [], |r| r.get(0))?;
    ...
    Ok(DashboardStats {
        ...
        by_status: pairs(
            conn,
            "SELECT status, COUNT(*) FROM job_status GROUP BY status ORDER BY 2 DESC",
        )?,
        // The reference calls this "by channel". Ours is which collector found
        // the job, which is the same question answered with data we hold
        // rather than a field a human types.
        by_source: pairs(
            conn,
            "SELECT COALESCE(source, 'unknown'), COUNT(*) FROM jobs
             WHERE verdict = 'keep' GROUP BY 1 ORDER BY 2 DESC",
        )?,
        employers_total: count(conn, "SELECT COUNT(*) FROM companies")?,
        // Readable means we resolved a job board we can actually collect from.
        // This is the number that explains a thin result set, and the
        // reference has no equivalent because it has no collector.
...
mod tests {
        ...
        conn.execute_batch(
            "INSERT INTO companies (id, name, ats) VALUES
             ...
             INSERT INTO jobs (key, company_id, title, verdict, qualified, source,
                               missing_skills, fetched_at, posted_at) VALUES
                ...
                ('lv:3', 3, 'Support', 'alt', 1, 'lever', '',
                 '2026-08-01T09:00:00', date('now','-2 day'));",
        )
        .unwrap();
        conn
    }
    ...
    fn counts_come_from_the_stored_data() {
        ...
        assert_eq!(module_count(&stats, crate::modules::Module::Alt), 1);
    ...
    fn accepting_an_offer_is_its_own_rung_between_offer_and_hired() {
        // The gap the first user asked for. A person who accepted and is waiting on a
