...
/// Written by the employer's side - verified by construction against
/// this module's own header note (POSTING-class content is kept away
/// from `brief` and anything else model-facing). Its contents are
/// never offered to one.
pub const POSTING: &str = "posting";

/// Refused at attach time rather than stored and guarded afterwards -
/// verified by construction: refuse_reason() below checks this list
/// and every write path (add_file, add_bytes) calls it before writing
/// anything - a file
...
impl Kind {
    ...
    ///
    /// A KIND THE APP RECOGNISES gets "Download to view" - verified by
    /// construction in the match arms below - it is a real
...
/// much as a path traversal. Verified by construction in
/// safe_display_name below and tested by
/// a_display_name_cannot_carry_its_own_instructions.
...
/// given - verified by construction below, this queries randomblob()
/// rather than reading `original`. A name chosen by whoever wrote the
/// file cannot then traverse a
...
/// Where a class of attachment lives. SEPARATE DIRECTORIES PER CLASS -
/// verified by construction, the join below keys on `trust` - so
...
///
/// Separate from add_file because there is no source file to copy -
/// verified by construction, this signature takes `data: &[u8]` rather
/// than a path: the person
...
///
/// THE SCHEME IS CHECKED HERE, at attach time - verified by construction
/// in the fmt::safe_link check two lines below - for the same reason a
/// file's
...
///
/// REFUSED RATHER THAN BLANKED, unlike the collectors - verified by
/// construction, the check below returns Err rather than clearing the
/// field. A collector handling a
...
///
/// THE BYTES MOVE TOO - verified by construction, the fs::rename call
/// below is what makes that true. The class is a directory, so a row that changed sides
/// while its file stayed put would leave a readable copy in the readable
/// folder - the protection would be a label and nothing else. Believed, not measured,
/// that the engine's copy of this had exactly that bug until a positive
/// control found it - no history under app/ to check that against.
...
mod tests {
    ...
    /// line break in a name is a new line in whatever reads it. Tested by
    /// this function itself, below.
    #[test]
    fn a_display_name_cannot_carry_its_own_instructions() {
        ...
        // called something clever. Verified by the assertion on this line.
