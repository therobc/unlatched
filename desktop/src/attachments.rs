...
// employer's side: the posting, a description PDF, a recruiter's email. The first user,
// 2026-08-12: "That's the only place I am currently wanting to restrict
// attachment access. Everything else needs to be AI accessible like resumes
// for writing." So POSTING-class content is kept away from `brief` and from
...
///
/// NOTHING IS RENDERED IN THE APP (Decided 2026-08-13: "Build attachments as
...
impl Kind {
    ...
    /// What hovering one says. The first user's two sentences, kept verbatim.
    ///
...
mod tests {
    ...
    /// The engine refuses a set of extensions and so does this file. A
    /// suffix added to one and not the other means the CLI door and the app
    /// door disagree about what is safe, which is exactly the kind of drift
