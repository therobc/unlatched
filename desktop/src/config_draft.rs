...
// and the nullable salary floor, are edited as plain text - verified by
// construction in the struct below (salary_floor, refresh_at etc are
// String fields) - so an in-progress edit (a half-typed number) never
// has to be a valid Config value. `to_config` turns the draft back into
// a real Config and reports every field that failed validation.
...
pub struct ConfigDraft {
    ...
    /// disk, so a field left out here would be written back empty. Tested
    /// by the_resume_pin_survives_a_save_from_this_screen.
    ...
    /// Times of day, as the person typed them. Held as text rather than
    /// parsed values - verified by construction, this field is a String -
    /// so a half-typed entry does not have to be valid mid-keystroke.
    pub refresh_at: String,
    /// The same, for Saturday and Sunday. Separate because the weekend
    /// run is a different decision, and text for the same reason. Tested
    /// by the_weekend_time_survives_a_round_trip_through_the_draft.
...
///
/// An empty work_modes reads as ALL THREE ticked rather than none,
/// verified by construction in the function below, because
/// that is what it means to the engine and a person opening this page
/// should see the search they actually have. Falls back to the
/// superseded remote_scope so a config written before the ticks
/// existed still shows remote-only as remote-only.
...
impl ConfigDraft {
    ...
    /// Parses every field, collecting one message per invalid field
    /// instead of stopping at the first problem - verified by construction
    /// in the errors.push() calls throughout the body below, none of which
    /// return early - so a Save attempt shows the user everything that
    ...
    pub fn to_config(&self) -> Result<Config, Vec<String>> {
        ...
        let refresh_at = match parse_times(&self.refresh_at) {
            ...
            Err(message) => {
                ...
                // chose - tested by a_rejected_edit_does_not_silently_move_the_schedule -
                // which for an instance sequenced behind another job is
        ...
        // to name the switch that would settle it. Tested by
        // an_empty_time_for_a_run_that_happens_names_its_own_way_out. Being
        // told to untick the daily refresh over a blank WEEKEND box is how
        // somebody turns the whole schedule off to clear a message about one
        // day of it.
        //
        // When the run is switched off, an empty list is stored and nothing is
        // blocked: tested by a_run_that_is_switched_off_does_not_need_a_time.
        // The engine reads an empty list as "use the documented
        ...
        // verified by construction in the `if !(all three)` guard below.
        ...
        // written back neutral - verified by construction, the line below
        // sets it unconditionally to "any". Leaving a "remote_only" in place
        // would keep filtering underneath ticks that say otherwise, with
        // nothing on screen to explain where the onsite roles went.
...
/// answer travels with the complaint - verified by construction: every
/// errors.push(...) site above interpolates {TIME_FORMAT} or {FORMAT}.
...
pub fn parse_times(raw: &str) -> Result<Vec<String>, String> {
    ...
    for piece in raw.split(',') {
        ...
        let (hour_text, minute_text) = match text.split_once(':') {
            Some(pair) => pair,
            // A bare number is an hour on the hour. Tested by
            // a_bare_hour_is_on_the_hour. Common enough to type that
    ...
    // A BLANK FIELD IS NOT THIS FUNCTION'S DECISION. Believed, not measured,
    // that it used to refuse here with one message about the daily refresh,
    // whichever box was empty - there is no history under app/ to check
    // that against. What is verified by construction, in to_config above:
    // the box that would satisfy such a message is disabled whenever the
    // run it schedules is switched off, so what an empty list means
    // depends on whether the run happens, which only the caller knows.
    out.sort_unstable();
    out.dedup();
    Ok(out
        .into_iter()
...
mod refresh_time_tests {
    ...
    fn a_time_that_is_not_on_the_clock_is_refused_by_name() {
        // The important half. Tested by the assertions directly below:
        // err.contains(bad) and err.contains("24-hour"). Silently keeping a
        // value that can never fire is indistinguishable from a working
    ...
    fn a_rejected_edit_does_not_silently_move_the_schedule() {
        ...
        // The saved config is never written when to_config errors - verified
        // by construction in app.rs's save_config, which only calls
        // config::save inside the Ok branch of this match - so the file
    ...
    ///
    /// A run that is switched off does not need a time - tested by this
    /// function itself, below. The engine already
    ...
    /// Tested by this function itself, below.
    #[test]
