...
//!
//! THE FLOW the first user settled on:
//!
//! ```text
//!   Applied -> No Offer
...
//! freeze lands between "I accepted" and "I started". The first user asked for that split
//! by name after we tried to make one status carry both.
...
const LEGACY: [(&str, &str); 1] = [("closed", "No longer open")];
...
/// keeping. `label()` already renders it as "No longer open" through LEGACY.
pub const CLOSED: &str = "closed";

/// The rename applied on open, in both halves of the app.
///
/// "Denied" was the app's word for it. The first user asked for "No Offer" - the same
...
mod tests {
    ...
    /// The dropdown scrolled for a second time, reported as "at one point we
    /// had all the statuses without having to scroll, but that issue is
    /// back". egui caps a combo popup at 200 px, which fits seven entries -
    /// and the entries that fall off the bottom are the ones somebody reaches
    /// for at the END of a search.
    ///
    ...
    fn the_two_easiest_to_confuse_are_never_side_by_side() {
        // the first user's rule, and the reason for it: "No Offer" and "Offer Withdrawn"
    ...
    fn the_legacy_value_still_reads_as_a_word() {
        assert_eq!(label("closed"), "No longer open");
