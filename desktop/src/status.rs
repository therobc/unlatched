...
pub struct Status {
    ...
    /// can vanish, without anybody ever applying. Verified by construction:
    /// FLOW's `pass` entry sets rung: None, below.
    ...
    /// asked. Hidden, never deleted - verified by construction: settled_values()
    /// below only filters what triage shows, and nothing in this module issues
    /// a DELETE.
    pub settled: bool,
...
/// list the same statuses in the same order - tested by
/// tests/test_status_vocabulary.py's
/// test_both_halves_list_the_same_statuses_in_the_same_order, which parses
/// the literal "pub const FLOW" marker. The two spellings are held
/// together by a test in this module instead.
...
/// and "Offer Withdrawn" are deliberately NOT adjacent. Verified by
/// construction below (they sit two apart) and tested by
/// the_two_easiest_to_confuse_are_never_side_by_side.
///
/// The three "ended without an offer" entries sit together at the end,
/// before Pass: they are what a person reaches for most often, and
/// grouping them makes the choice between them the visible decision rather
/// than a hunt.
pub const FLOW: [Status; 11] = [
    ...
    Status {
        ...
        // describing something that never happened. Tested by
        // offer_withdrawn_reads_the_history_not_the_current_status and
        // a_dependent_status_is_blocked_with_a_reason_rather_than_missing.
...
/// The status the app writes when a posting is taken down and the person
/// had never acted on it.
///
/// NOT IN `FLOW`, on purpose - verified by construction: "closed" does not
/// appear in the FLOW array above, so it cannot be chosen from any
/// dropdown and never competes with the statuses a person sets. It is
/// only ever written by `db::mark_taken_down`, and only onto rows
/// carrying no decision worth keeping. `label()` renders it as "Expired"
/// through LEGACY.
...
///
/// egui's default cap is 200 px - verified in egui 0.28.1's own source,
/// Style::default() setting combo_height: 200.0 (style.rs:1156), roughly
/// seven entries - and the list has been longer than that for a while,
/// which quietly hid the statuses at the END of a search behind a
/// scrollbar. Computed from the live entry count and the real row height
/// so that adding a status widens the popup rather than hiding one.
/// Tested by the_status_popup_is_tall_enough_for_every_entry.
...
/// The arithmetic on its own, with no `Ui` to build, tested directly by
/// the_status_popup_is_tall_enough_for_every_entry - so the rule that
/// matters ("every entry fits") can be asserted in a unit test rather
/// than only seen. Same split as the engine's other UI-adjacent maths.
pub fn popup_height_for(entries: usize, row: f32, gap: f32, margin: f32) -> f32 {
    row * entries as f32 + gap * (entries as f32 - 1.0) + margin
}

/// egui's own default cap on a combo popup - verified in egui 0.28.1's
/// source, Style::default()'s combo_height: 200.0 (style.rs:1156). Named
/// here because the whole point of `popup_height` is to be taller than it.
#[cfg(test)]
...
/// How a status reads. Falls back to capitalising whatever it was handed,
/// so a hand-edited database or an import from a future version still
/// renders a word rather than a raw token. Tested by
/// the_legacy_value_still_reads_as_a_word
/// (label("second_interview") == "Second interview").
...
///
/// Mid-tone on purpose: believed, not measured, that these read against
/// both a near-white panel and a near-black one - no contrast ratio has
/// been checked - but the app does not have to keep two palettes in step.
...
/// promised "interviewed, offer, hired or denied" - naming a status this
/// app stopped writing, and omitting the four it had gained since.
/// Verified by construction: the function below is built directly from
/// FLOW.iter(), not retyped.
...
/// entry somebody is looking for reads as a bug in the app; a greyed
/// entry that says why reads as the app knowing something.
///
/// HISTORY, NOT CURRENT STATUS. Tested by
/// offer_withdrawn_reads_the_history_not_the_current_status: a job
/// marked Accepted Offer, then Hired, where the hire then falls
/// through - the current status is Hired and carries no trace of the
/// acceptance, but the history does, and Offer Withdrawn stays
/// available for exactly that person.
...
mod sort_rank_tests {
    ...
    /// Tested by the assertions directly below.
    #[test]
    ...
    /// second place to forget. Tested by this function itself, immediately
    /// below.
    #[test]
...
mod tests {
    ...
    /// drifting apart - tested by this function's own assertions, directly
    /// below.
    #[test]
    ...
    /// POPUP_ENTRIES is what the height is computed from, so if it drifts
    /// from what the dropdown actually renders the height is right for the
    /// wrong list. The dropdown draws every FLOW status plus "not set".
    /// Tested by this function's own assertion, directly below.
    #[test]
    ...
    fn the_two_easiest_to_confuse_are_never_side_by_side() {
        ...
        // search got. Verified by construction: they sit two positions apart in
        // FLOW above (offer_withdrawn at index 5, no_offer at index 7). A future
        // reorder that puts them together has to fail here rather than in front
        // of a person marking the end of a job hunt.
    ...
    fn the_sql_list_quotes_what_the_queries_interpolate() {
        ...
        // Verified by construction: FLOW's literals above contain no quote.
