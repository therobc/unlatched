...
// lie about what a profile was, and duplicated the resume. Verified by
// construction: the Person/Registry structs below are exactly the two
// levels this describes.
//
...
// point at - verified by construction in registry_path below, which reads
// no environment variable in either build configuration - see the
...
// never a name a user can register under - verified at
// new_profile.rs:159, which refuses both DEFAULT_PROFILE and this
// constant.
...
pub struct Person {
    ...
    /// an optimisation against one search. Verified by construction: this
    /// field is an owned Option<String> per Person, not a shared handle.
...
///
/// This is a hard guard, not tidiness. Believed, not measured, that
/// `remember_resume` used to save() as a
/// side effect of mutating and once replaced a live
/// registry of five seekers with a test fixture - no history under app/
/// to check that incident against. Verified by construction below:
/// remember_resume no longer calls save() at all.
#[cfg(test)]
pub fn registry_path() -> PathBuf {
...
///
/// Not AppData, which works but is hidden - wrong for files somebody needs to
/// back up, copy to a new machine, or keep with the rest of their job hunt.
/// Never Program Files - verified by construction, the match below never
/// names it.
///
...
/// filesystem - verified by construction, this function takes only a
/// &str and makes no fs call - the migration is the part that would
/// silently lose somebody's profiles, so it is the part that has to be
/// provable.
...
/// for anything missing from the map, verified by construction in the
/// unwrap_or_else below, so the app always has somewhere valid to
...
///
/// Offered, never imposed - verified by construction, this is a pure
...
/// typed naturally and still become a directory. Verified by
/// construction in the char map below.
...
/// renamed everybody. Verified by construction in the equality check below.
...
///
/// Overwrites an existing entry of the same pair - verified by
/// construction, `searches.insert` below replaces any existing value
/// for that key: re-running "New search" with
...
/// left untouched: deleting from the UI only unregisters.
///
...
///
/// Mutates ONLY - verified by construction, no save() call appears in
/// the body below. Believed, not measured, that this
/// used to call save() itself and that a unit test once silently
/// replaced a live registry of five seekers with its own fixture - no
/// history under app/ to check that incident against. Nothing in this
/// module writes to disk unless a caller asks it to.
///
...
pub fn remember_resume(reg: &mut Registry, person: &str, resume: Option<&str>) {
    // or_default so this works for a person who does not exist yet -
    // verified by construction in the line below. It is
    // called before register_and_activate (verified at
    // new_profile.rs:199-200) precisely so that one save persists
...
///
/// Believed, not measured, that both failure modes here have already
/// happened silently and cost real time: two seekers had complete
/// configs, resumes and employer
...
/// straight over with "no database, skipping" - no history under app/ to
/// check that against. Verified by construction and tested by
/// preflight_names_a_search_whose_folder_is_gone: the checks below cover
/// exactly these two shapes. A registry problem should be loud.
...
///
/// An EXISTING config.json is never replaced with defaults - verified by
/// construction below, an existing file is loaded and modified rather
/// than overwritten with Config::default(). Pointing a new search at a
/// folder that already holds someone's work would otherwise reset
/// their settings to defaults, which is data loss for the exact
...
mod tests {
    ...
    const V1: &str = r#"{
        "active": "sample",
        "profiles": {
            "Dana Whitfield": "C:/example/hr",
            "Darius Webb": "C:/example/logistics",
            "sample": "C:/example"
        }
    }"#;
    ...
    fn preflight_names_a_search_whose_folder_is_gone() {
        ...
        // Every path in V1 is fictional, so all three are reported: verified
