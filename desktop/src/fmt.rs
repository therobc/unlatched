...
pub fn salary_range(
    ...
    // by 2080 hours - our assumption, not the employer's. The match below is
    // why the rate leads and the annual equivalent follows in parentheses,
    // marked approximate: by construction the hourly branch returns before
    // min/max are ever consulted.
...
pub fn collected_line(last_collected: Option<&str>) -> String {
    ...
    // NAMES THE BOARDS rather than a bare "Collected today", because a
    // claim this size should say what it is about - the board sweep runs
    // whenever the app runs and may say nothing about an external
    // collector. Believed, not measured, that an earlier version said
    // less than this; there is no history under app/ to check it against.
    if age.is_empty() {
...
pub fn collector_file_line(
    ...
    let age = match age_hours {
        ...
        // Unstamped is not fresh: tested by
        // collector_file_tests::an_unstamped_file_does_not_read_as_fresh,
        // which asserts this reads "age unknown" with attention set true.
    ...
    //
    // An hour of tolerance (the `rows - file > 1.0` below): the file's time
    // comes from the filesystem and the rows' from a stamp the importer
    // wrote, so they are never exactly equal. Tested by
    // collector_file_tests::matching_ages_are_not_reported_as_waiting.
...
pub enum Collected {
    ...
    Today,
    /// Nothing has arrived yet today. This variant carries no warning.
    NotYet,
...
///
/// The collector finishes by 12:30, so lateness is a TIME OF DAY rather
/// than a duration: tested by
/// lateness_tests::a_delivery_made_today_is_never_late_today and
/// lateness_tests::yesterdays_delivery_is_late_this_afternoon.
///
/// An hour of grace past the usual finish - see GRACE_SECS in
/// is_late_at below.
///
/// A stamp-less source is never "late": tested by
/// lateness_tests::a_source_that_never_delivered_is_never_late.
...
mod collector_file_tests {
    ...
    fn a_missing_file_says_so_instead_of_showing_an_age() {
        ...
        // thing to say. Tested below: the text omits an age and attention
        // is set true.
    ...
    fn a_file_older_than_a_day_wants_attention() {
        // The collector runs daily. Tested below: 23h old reads as fine and
        // 30h old wants attention, either side of that line.
...
mod collected_line_tests {
    ...
    fn the_line_says_which_collector_it_is_about() {
        // It used to read "Collected today" full stop, over a number
        // covering every source - believed, not measured, since there is no
        // history under app/ to check it against. The assertions below
        // confirm today's line names "boards".
...
mod lateness_tests {
    ...
    fn a_delivery_made_today_is_never_late_today() {
        // 15:00, handover landed at 12:07. Believed, not measured, that this
        // matches what the real collector did the day this was built - no
        // history under app/ to check it against. 12:07 is before the 12:30
        // deadline, so the assertion below expects "not late" three hours on.
    ...
    fn yesterdays_delivery_is_late_this_afternoon() {
        // Newest handover is from 12:07 YESTERDAY: today's stamp never came,
        // so the assertion below expects is_late_at to return true.
        // Believed, not measured, that this is exactly the failure the
        // dashboard panel exists to catch.
    ...
    fn a_source_that_never_delivered_is_never_late() {
        // Not late, unused. Verified in dashboard_view.rs: source_chart
        // gates source_is_late on `external`
        // (`stale: external && source_is_late(...)`) - only configured
        // collectors are asked at all - but a stamp that cannot be read
        // must not turn into a warning either.
...
///
/// SEPARATE FROM posted_age because the resolution differs: that one
/// answers "how old is this posting", where a day is the smallest
/// unit anyone acts on; this answers "is this feed alive". Tested by
/// lateness_tests::an_undatable_stamp_reads_as_never_not_now, and the
/// match below returns "never" for anything unparseable, by construction.
...
/// True once a posting is old enough that it is often already filled.
/// Used only to soften the row, never to hide or drop it: its one
/// caller (triage.rs's TriageRow rendering) only ever applies `.weak()`
/// styling, by construction never filters anything out.
...
/// Age of a posting in days, from whatever the board wrote. Verified
/// its one caller today is app.rs's SortBy::Posted (the triage row's
/// own sort) - the dashboard's fresh_matches count this comment used
/// to also credit was removed in favour of a database WHERE clause
/// (see dashboard.rs's own note on that), so this is no longer
...
pub fn link_host(url: &str) -> String {
    ...
    let host = authority
        ...
        .map(|(_, host)| host)
        .unwrap_or(authority);
    // An IPv6 literal is bracketed and full of colons, by definition of
    // URI host syntax, so the port can only be stripped after the
    // closing bracket. Tested by
    // link_host_tests::a_port_is_not_part_of_the_host.
...
///
/// egui's `hyperlink_to` ends at `webbrowser::open`, which is the
/// Windows shell - so whatever reaches it gets opened with the
/// handler registered for its scheme. Job URLs arrive from remote
/// JSON-LD, so without this a posting could nominate
/// `file://198.51.100.5/share/x` and one click on the job title would
/// hand over the person's NTLM credentials - found by a red-team
/// review, 2026-08-08 (see unlatched/links.py for the matching note
/// on the engine side).
///
/// Both halves now refuse to STORE such a URL - the engine's
/// collectors and hand-add path, and `attachments::add_link` on this
/// side - but this check is not redundant: databases collected before
/// those fixes already contain whatever was offered, and this is the
/// boundary that actually touches the OS. Tested by
/// link_host_tests::only_http_is_a_link.
...
mod link_host_tests {
    ...
    fn userinfo_cannot_impersonate_a_host() {
        ...
        // greenhouse, the click went to evil.com. The assertion below
        // confirms link_host now returns the real host ("evil.com"), not
        // the spoofed userinfo. Believed, not measured, that the column
        // clips long text the way an attacker would rely on.
        assert_eq!(
