...
impl UnlatchedApp {
    ...
    fn report_add_job(&mut self, label: &str, code: i32) {
        ...
        // STDERR FIRST, because that is where the engine puts a refusal and
        // the last line printed is not reliably the reason: `add` runs
        // duplicate grouping AFTER the failing step, so a successful regroup
        // reports last and reads as the cause. Observed 2026-09-07: the
        // modal blamed "1 group(s) now show the still-open
        // posting" for a failure whose real reason, no title on the page, was
        // sitting in the status bar at the same moment.
        //
        // The marker is written by process.rs, which prefixes every stderr
        // line, and stripped here so the person reads the sentence rather than
        // the plumbing.
        let reason = self
            .log_lines
            .iter()
            .rev()
            .find(|line| line.starts_with(STDERR_MARK))
            .map(|line| line[STDERR_MARK.len()..].trim().to_string())
            .or_else(|| {
                // Nothing on stderr: fall back to the old rule rather than
                // saying nothing. Believed, not measured - no failing add has
                // been seen that wrote nothing to stderr, and this branch is
                // here so that one would still get a sentence rather than the
                // generic line.
                self.log_lines
                    .iter()
                    .rev()
...
/// What process.rs prefixes to every line a child wrote to stderr.
/// Named here rather than spelled inline so the two files cannot drift.
pub const STDERR_MARK: &str = "[stderr] ";
