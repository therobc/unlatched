...
pub const STEPS: [Step; 11] = [
    ...
    Step {
        ...
        body: "If you have an AI assistant on this computer - one that runs in \
               your editor and can use a terminal - tell it: \"There is a CLI \
               for my job search app. Run: unlatched --help\".\n\nIt can then \
...
mod tests {
    ...
    /// The row title is a hyperlink to the employer's site, so "click a row to
    /// read the posting" sent people out to a browser - the exact trap that
    /// cost a harness debugging session on 2026-08-12.
    #[test]
