...
mod tests {
    ...
    /// Serialises every test that writes the process-wide offset.
    ///
    /// set_local_offset stores into a static, and cargo runs tests in parallel
    /// threads - so without this one test sets -04:00 while another asserts on
    /// +05:30. Observed as an intermittent failure of
    /// a_western_half_hour_zone_formats_correctly on 2026-09-06, passing on a
    /// re-run, which is the shape of flake that teaches people to re-run until
    /// green.
    ///
    /// Poisoning is deliberately ignored: a panic in one of these tests has
    /// already failed that test, and refusing to run the others afterwards
