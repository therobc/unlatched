...
def test_the_document_ships_with_the_app():
    """It is in app/, which is the tree that is published. A contract that
    lives only in an internal document is one no adopter can read."""
...
def test_the_first_character_rule_is_stated_as_well_as_enforced(text):
    """The first character is narrower than the rest, and the document used
    not to say so.

    An underscore or a hyphen is legal anywhere in an id EXCEPT the front,
    which the prose above this test's subject described as a flat character
    set. A stranger reading it would write `_myboard` and have it refused
    with no hint from the document that it could be.
    """
    assert "start\nwith a letter or a digit" in text or \
           "start with a letter or a digit" in text, \
        "COLLECTORS.md does not state the first-character rule"

    for bad in ("_leading", "-leading"):
        with pytest.raises(importer.BadCollectorIdError):
...
def test_the_hand_back_file_it_documents_is_the_one_written(text):
    """The return direction, undocumented until this review.

    The filename IS the interface on that side - a collector author has
    nothing else to look for - so it is derived here from the function that
    builds it rather than copied from the document.
    """
    from unlatched import closures, collectors

    entry = collectors.enabled({"collectors": [
        {"id": "x", "path": "C:/nowhere/inbox.json"}]})[0]
    assert closures.default_path(entry).name == "inbox-closed-by-unlatched.json"

    assert "-closed-by-unlatched.json" in text, (
        "COLLECTORS.md does not name the hand-back file, so nothing tells a "
        "collector author it exists")
    assert f'"version": {closures.VERSION}' in text
...
def test_an_empty_schedule_still_means_every_refresh(text):
    """The one place this document departs from the specification it was
    written against. If the default is ever flipped, this fails and the
    document is corrected with it rather than after somebody's daily pull
    stops."""
