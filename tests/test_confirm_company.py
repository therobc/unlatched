...
def test_initialism_confirms_when_every_other_word_is_generic():
    ...
    assert discover.page_confirms_company(page, "NWS Financial Services")
    assert discover.page_confirms_company(
        "<html>FBR is hiring</html>", "FBR Logistics")


def test_initialism_must_match_as_a_whole_word():
    """"nws" inside "nwsbank" is not evidence the page is that employer's -
    the same strictness that keeps "abb" from matching "abbey".
    """
    assert not discover.page_confirms_company(
        "<html>nwsbank-lookalike.example</html>", "NWS Financial Services")
...
def test_a_wrong_company_page_still_fails():
    assert not discover.page_confirms_company(
        "<html>welcome to acme corp</html>", "FBR Logistics")
...
# ---- greenhouse publishes its board five different ways -----------------

def test_every_live_greenhouse_embed_form_is_fingerprinted():
    """The original pattern covered two of five. One employer publishes the
    "/js" embed variant and another the bare "boards.greenhouse.io/slug";
    both were recorded as having no board while being fully collectable.

    THE FIFTH IS THE APPLICATION FORM, and it is the one a person is most
    likely to be holding: an aggregator's "original job post" link goes to
    /embed/job_app, because that is the page you apply on rather than the
    employer's board index. Measured 2026-09-02 against a live link of that
    shape - the slug was sitting in the query string and came back empty.
    """
    forms = {
        "//boards.greenhouse.io/embed/job_board/js?for=northwindhealth":
            "northwindhealth",
        "//boards.greenhouse.io/fabrikam": "fabrikam",
        "https://boards.greenhouse.io/embed/job_board?for=acme": "acme",
        "https://job-boards.greenhouse.io/acmecorp": "acmecorp",
        # Exactly the shape handed over by an aggregator, tracking parameters
        # and all - those must not stop the slug being read.
        "https://job-boards.greenhouse.io/embed/job_app?for=contosohealth"
        "&jr_id=6a7f3d93927c79391ad0745e&token=8499256002&utm_source=elsewhere":
            "contosohealth",
        "https://boards.greenhouse.io/embed/job_app?for=fourthcoffee":
            "fourthcoffee",
    }
