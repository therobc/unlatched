...
Why, decided 2026-08-12: ordinary people do not know what JSON is, and a
format somebody cannot read is a format they cannot produce. This app already
EXPORTS csv, so accepting it inbound also gives round-trip symmetry: export,
edit in Excel, hand it back.

THE EARLIER CASE AGAINST CSV AIMED AT THE WRONG RISK. read_rows' old docstring
said a multi-paragraph description "is where CSV goes wrong quietly". That is
true of HAND-ROLLED writers and false of Excel and Python's csv module, both of
which quote embedded newlines and quotes correctly. The first test below is that
claim measured rather than repeated.
"""
...
def test_a_closed_row_is_a_closure_and_not_a_job(tmp_path):
    """A `closed` column carries them in the same file, which is arguably
    tidier than JSON's separate list. Importing such a row as well
