...
If the two formats disagree, one of them is wrong - and the person who finds
out is whoever wrote the file, after their rows have already landed wrong.
...
JOBS = [
    {"key": "a1", "url": "https://boards.greenhouse.io/acme/jobs/1",
     ...
     "apply_kind": "external"},
    {"key": "a2", "url": "https://www.example.com/jobs/view/222",
...
COMPARED = ("key", "source", "title", "location", "url", "apply_url",
            "apply_kind", "posted_at", "description",
            "qualified", "verdict", "score")

# delisted_at is DELIBERATELY NOT COMPARED, and this test taught that lesson
# the expensive way. It is stamped with the clock at import, and the two
# imports below run one after the other - so whenever they straddle a second
# boundary the two rows differ by one second and the comparison fails. That is
# the intermittent failure this suite carried for days: unreproducible on
# demand, more likely while the machine was busy because a slower import is
# likelier to cross a second, and nothing to do with the formats it exists to
# compare. Caught 2026-08-27 with the two values one second apart.
#
# Whether the closure LANDED is still asserted, below, on both sides. What is
# dropped is only the claim that two clock readings are equal.
STAMPED_AT_IMPORT = ("delisted_at",)
...
def landed(home, path):
    ...
    try:
        ...
        # WHOLE ROWS. The caller narrows to COMPARED for the equality check
        # and reads the stamped fields separately - see STAMPED_AT_IMPORT.
        rows = [dict(r) for r in con.execute("SELECT * FROM jobs ORDER BY key")]
...
def test_the_same_payload_in_either_format_lands_the_same_board(tmp_path,
    ...
    # THE STAMPED FIELDS, checked for presence rather than equality - see
    # STAMPED_AT_IMPORT. The closure still has to have landed on both sides,
    # and on the row that was closed and no other.
    for field in STAMPED_AT_IMPORT:
        assert ({r["key"] for r in rows_json if r[field]}
                == {r["key"] for r in rows_csv if r[field]}
                == {"partner:a1"}), field
