...
def _descriptor(position_id: str = "ORNL-1", title: str = "Nuclear Engineer",
                 **overrides: Any) -> dict[str, Any]:
    base: dict[str, Any] = {
        "PositionID": position_id,
        "PositionTitle": title,
        "PositionURI": f"https://www.usajobs.gov/job/{position_id}",
        "PositionLocationDisplay": "Oak Ridge, Tennessee",
...
def _item(**overrides: Any) -> dict[str, Any]:
    return {"MatchedObjectId": overrides.get("PositionID", "ORNL-1"),
...
def test_response_maps_to_job_fields():
    ...
    assert job.source_id == "ORNL-1"
    assert job.title == "Nuclear Engineer"
    assert job.location == "Oak Ridge, Tennessee"
    assert job.url == "https://www.usajobs.gov/job/ORNL-1"
    assert job.posted == "2026-08-01"
    assert job.employer == "Oak Ridge National Laboratory"
...
def test_location_falls_back_to_position_location_array_when_display_is_absent():
    item = _item(PositionLocationDisplay="", PositionLocation=[
        {"LocationName": "Oak Ridge, Tennessee"},
        {"LocationName": "Knoxville, Tennessee"},
    ])
    jobs = usajobs.collect(_cfg(), fetcher=_single_page_fetcher([item]))
    assert jobs[0].location == "Oak Ridge, Tennessee; Knoxville, Tennessee"
...
def test_collect_runs_usajobs_after_the_company_loop_and_creates_an_agency_company(
    ...
    cfg = _cfg(title_include=["nuclear engineer"], locations=["Oak Ridge, TN"])
