...
def test_discover_fingerprint_extracts_host_and_site():
    ...
    <html><body>
      ...
      <p>Search open roles at
        <a href="https://eluq.fa.us2.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1/requisitions">
          our jobs board</a>.
      </p>
    </body></html>
    """
    found = discover.detect_ats(html)
    assert {"provider": "oracle_hcm", "parts": ["eluq.fa.us2.oraclecloud.com", "CX_1"]} in found


def test_discover_fingerprint_handles_a_non_numeric_site_slug():
    html = ('<a href="https://emcm.fa.us2.oraclecloud.com/hcmUI/CandidateExperience/'
            'en/sites/WMCareers/requisitions">Careers</a>')
    found = discover.detect_ats(html)
    assert {"provider": "oracle_hcm",
            "parts": ["emcm.fa.us2.oraclecloud.com", "WMCareers"]} in found
...
def test_the_precise_fingerprint_is_preferred_when_both_could_match():
    ...
    html = ('<a href="https://example.fa.us2.oraclecloud.com/hcmUI/'
             'CandidateExperience/en/sites/WMCareers/requisitions">jobs</a>')
    found = [f for f in discover.detect_ats(html) if f["provider"] == "oracle_hcm"]
    assert found[0]["parts"] == ["example.fa.us2.oraclecloud.com", "WMCareers"]
