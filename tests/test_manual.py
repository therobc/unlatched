...
def test_off_means_nothing_is_read(home):
    """Off means nothing on this path is read - not "read the ordinary sites
    but not the restricted ones", which is what the OLD name implied while the
    switch only gated one host. The name says what it does now.

    TURNED OFF EXPLICITLY HERE, because the default no longer is. It shipped
    off until 2026-09-07 and this test used to lean on that, which quietly
    made it two tests: what the default is, and what off does. They are now
    separate, so changing the default cannot take the guarantee with it."""
    con = db.connect(home)
    cfg = config.defaults()
    cfg["fetch"]["read_added_links"] = False
