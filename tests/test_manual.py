...
Decided 2026-08-06: he was applying to a job he had found on LinkedIn and
wanted it in his pipeline. The feature has to work for that WITHOUT the app
going and fetching LinkedIn, which is the thing these tests mostly pin down.
"""
...
{"@context":"https://schema.org/","@type":"JobPosting",
 ...
 "jobLocation":{"address":{"addressLocality":"Knoxville","addressRegion":"TN"}},
...
<a class="topcard__org-name-link" href="/company/x">GoFundMe</a>
...
# ---- attended-only hosts, which are a decision by the first user ----------------
#
...
def test_an_attended_only_host_is_read_only_with_a_person_present():
    ...
    url = "https://www.linkedin.com/jobs/view/4450618917/"
...
def test_it_ships_off_and_off_means_nothing_is_read(home):
    """Shipped default, the first user 2026-08-08: it goes out off because this is public
    under someone else's name, not because the behaviour is indefensible.
...
def test_a_blank_link_is_refused(home):
    ...
    for bad in ("", "   "):
        try:
            manual.add(con, config.defaults(), bad, title="x", fetcher=make_fetcher({}))
        except ValueError:
            pass
        else:
