...
Delete, over a multi-select, decided 2026-08-08. It is implemented as
retire-and-restore: the row leaves every list immediately, which is the
result that was wanted, but nothing is destroyed. A bulk action over a multi-select is one
...
def test_a_finished_application_still_counts_as_applied_to(con):
    ...
    "denied", which was renamed to "no_offer" and now only arrives by
    migration from an older database.
    """
    add_job(con, "gh:1")
    status.set_status(con, "gh:1", "applied")
    status.set_status(con, "gh:1", "no_offer")
