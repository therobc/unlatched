...
def test_same_title_different_cities_is_not_a_repost():
    """A freight carrier posts "Class A CDL Local Driver" at terminals in
    dozens of cities. Grouping on company+title alone called that 18 reposts
    of one job.
    """
    a = reposts.seat_key("Ridgeway Freight", "Class A CDL Local Driver", "Dallas, TX")
    b = reposts.seat_key("Ridgeway Freight", "Class A CDL Local Driver", "Toledo, OH")
    assert a != b


def test_same_seat_matches_across_punctuation_and_case():
    a = reposts.seat_key("Vantor", "Logistics Specialist", "Naperville, Illinois")
    b = reposts.seat_key("vantor", "logistics  specialist", "Naperville Illinois")
    assert a == b
...
def test_named_place_is_not_vague():
    assert not reposts.is_vague(reposts.seat_key("Acme", "Analyst", "Dayton, OH"))
...
def test_a_long_gap_is_not_hidden_by_a_later_short_one(con):
    """One seat in the corpus went 41 days, then 3. Reading only the last
    interval called the whole thing a re-listing.
    """
    cid = db_mod.upsert_company(con, "Acme")
...
def test_vague_seats_are_never_reported_as_reposts(con):
    """Two unrelated openings at one employer, both filed under "Multiple
    Locations", were reported as one seat re-advertised after 192 days.
    """
    cid = db_mod.upsert_company(con, "Acme")
