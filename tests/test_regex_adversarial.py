...
# WHAT THIS BUDGET IS FOR, and why it is generous. The failure it catches is
# catastrophic backtracking, which is not a slow regex - it is an exponential
# one: the pattern this suite was written for took 1.5s at 20KB and 109s at
# 160KB. Anything healthy finishes these inputs in milliseconds, so the gap
# between pass and fail is three orders of magnitude and the exact number here
# changes nothing about what is detected.
#
# It was 1.0s, which is close enough to real work to be a claim about the
# MACHINE rather than the code, and a busy machine could fail it while nothing
# was wrong. Raising the ceiling costs no detection either way.
#
# NOT THE INTERMITTENT FAILURE THIS SUITE HAD. That one was found on
# 2026-08-27 and was not a timing budget at all: test_handoff_formats_agree
# compared two rows including a field stamped from the clock at import, so it
# failed whenever the two imports straddled a second. This budget was widened
# on suspicion; the suspicion was wrong and the widening is kept on its own
# merits.
TIME_BUDGET_S = 5.0
...
def test_attended_page_extraction_is_linear_time():
    ...
    html = _unclosed_markup()
    start = time.perf_counter()
    assert manual.read_attended_page(html) == {}, "nothing here is a real posting"
    elapsed = time.perf_counter() - start
    assert elapsed < TIME_BUDGET_S, f"read_attended_page took {elapsed:.3f}s on {len(html)} bytes"


def test_attended_page_extraction_still_reads_a_real_page():
    """A linear scanner that finds nothing would also pass the test above."""
    html = ('<html><body><h1 class="top-card-layout__title">Support Analyst</h1>'
            '<a class="topcard__org-name-link" href="/x">Example Ltd</a>'
            '<span class="topcard__flavor--bullet">Dayton, OH</span>'
            '<div class="show-more-less-html__markup">Answer tickets.</div>'
            '</body></html>')
    page = manual.read_attended_page(html)
    assert page["title"] == "Support Analyst"
    assert page["employer"] == "Example Ltd"
    assert page["location"] == "Dayton, OH"
