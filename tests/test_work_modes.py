...
def test_screening_keeps_exactly_the_ticked_modes(modes, expected_kept):
    postings = {
        ...
        "onsite": "Join our Dayton team. " + "Support work. " * 40,
    }
    cfg = config.defaults()
    ...
    for name, description in postings.items():
        result = screen.screen_job(_job("Dayton, OH", description), cfg)
        if result["qualified"]:
            kept.add(name)
...
def test_a_hybrid_role_is_checked_against_the_places_you_can_work():
    ...
    cfg = config.defaults()
    cfg["search"]["title_include"] = ["analyst"]
    cfg["search"]["locations"] = ["Dayton, OH"]
