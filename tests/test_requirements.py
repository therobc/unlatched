...
def test_requirements_verb_json_through_cli(tmp_path, monkeypatch):
    home = tmp_path / "home"
    monkeypatch.setenv("UNLATCHED_HOME", str(home))
    con = db.connect(home)
    db.upsert_job(con, "test:1", {
        "title": "Delivery Driver", "location": "Knoxville, TN",
...
def test_show_includes_requirements_summary(tmp_path, monkeypatch, capsys):
    ...
    db.upsert_job(con, "test:2", {
        "title": "Warehouse Associate", "location": "Knoxville, TN",
