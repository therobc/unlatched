...
The scheduled run is meant to be a refresh AND a dedupe (2026-08-09), and it
was only the first: `refresh` ran `collect` and stopped, so duplicates were
...
def test_two_boards_on_one_application_are_grouped_without_anybody_asking(con, home):
    ...
    add(con, "workable:1", "Support Analyst", company="Northwind",
...
def test_the_refresh_command_also_rebalances_a_group_whose_keeper_closed(con, home):
    ...
    # The kept MyBoard ad comes down; the ATS requisition stays live.
