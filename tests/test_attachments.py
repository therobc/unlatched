...
Decided 2026-08-12: employer-written attachments must not be readable by a
model, because that is a prompt-injection and malicious-execution path. Then,
narrowing it: that is the ONLY place attachment access is restricted, and
everything else stays reachable by an assistant - resumes above all.
...
def test_a_link_that_is_not_http_is_refused(con, job):
    """The store boundary for links, applied at the one route that skipped it.

    db.py calls this the store boundary and applies it to every collector and
    to the hand-add path: "anything that is not plain http/https loses its
    link at the one point every collector and the hand-add path both pass
    through". An attachment was the route that did not pass through it, so a
    javascript: or file: URL could be stored beside a job.

    REFUSED, not blanked. A collector handling a thousand postings drops a bad
    link and keeps the job; a person attaching one link typed it deliberately
    and is owed an answer - the same call manual.add makes.
    """
    for bad in ("javascript:alert(1)",
        ...
        with pytest.raises(attachments.Refused):
            attachments.add_link(con, job, bad, "Looks fine",
                                 attachments.MINE, "2026-08-12T10:00:00Z")
        assert attachments.list_for(con, job) == [], (
            f"{bad!r} was stored despite being refused")


def test_an_ordinary_link_still_attaches(con, job):
    """The positive control. A guard that refused everything would satisfy the
    test above perfectly and break the feature."""
    for good in ("https://acme.example/interview",
                 "http://careers.example.invalid/apply"):
        row = attachments.add_link(con, job, good, "Link", attachments.MINE,
                                   "2026-08-12T10:00:00Z")
        assert row["url"] == good
