"""The version is written in more than one file - verified 2026-09-10:
test_every_file_that_states_the_version_states_the_same_one below compares
all four - so they have to agree.
...
# Unverified history: believed, not measured, that these two were not
# covered for this test's first month and one drifted 28 releases
# while it watched the other two. __init__ is the worse of the two:
# verified 2026-09-10, fetch.py's USER_AGENT is built from
# unlatched.__version__, so a stale value is not a display problem,
# it is what this app tells every employer's server it is.
PACKAGE = APP / "unlatched" / "__init__.py"
CARGO = APP / "desktop" / "Cargo.toml"

# The documents a reader is handed. These were never in this test's
# scope. Unverified history: README.md is believed to have carried
# `Unlatched-Setup-0.1.1.exe` - the exact literal from the incident
# above - for twenty-eight releases after it.
...
def _one(pattern: str, path: Path) -> str:
    ...
    """
    # MULTILINE, because every pattern here is anchored at the start of a
    # LINE and without it `^` only matches the start of the file, by
    # definition of Python's re module. Unverified history: believed, not
    # measured, that the first version of this test reported "found 0"
    # against a file that plainly states its version.
...
def test_no_document_names_an_artifact_with_a_version_in_it():
    """Verified 2026-09-10: a filename in prose cannot be bumped by the build
    that renames the file - build_release.py copies README.md with
    shutil.copy2 rather than editing its text.
...
def test_that_search_would_find_a_versioned_filename():
    """A positive control, by construction: the assertions below show the same
    regex matching a real versioned filename, since the check above is a
    search for something ABSENT and would pass just as happily against a
