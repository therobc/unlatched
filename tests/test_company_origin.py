...
A "refresh the seeded companies" action cannot be expressed without this:
starter.seed() and discover BOTH write probe_status="yielding", so a shipped
employer and one the app found by crawling were indistinguishable.
...
def test_imported_employers_are_their_own_kind(con):
    ...
    origin = con.execute(
        "SELECT origin FROM companies WHERE name = 'From a handoff'").fetchone()[0]
