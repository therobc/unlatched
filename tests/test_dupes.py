...
def test_two_boards_pointing_at_one_application_are_one_job(con):
    add(con, "li:1", "Support Analyst", fetched="2026-08-02", company="Northwind",
        apply_url="https://apply.workable.com/northwind/j/ABC123/")
    add(con, "workable:1", "Support Analyst", fetched="2026-08-01", company="Northwind",
        apply_url="https://apply.workable.com/northwind/j/ABC123")
...
def test_a_republisher_does_not_become_the_preferred_route(con):
    """The caveat. A republisher's listing is an intermediary, not a
    ...
    add(con, "li:1", "Support Analyst", fetched="2026-08-02", company="Repostr",
        apply_url="https://apply.workable.com/northwind/j/ABC123/")
    add(con, "workable:1", "Support Analyst", fetched="2026-08-01", company="Northwind",
        apply_url="https://apply.workable.com/northwind/j/ABC123")
...
def test_a_hand_added_job_from_any_site_is_not_the_preferred_route(con):
    ...
    add(con, "manual:1", "Analyst", fetched="2026-08-01", company="Northwind",
        url="https://apply.workable.com/northwind/j/ABC123/",
        apply_url="https://apply.workable.com/northwind/j/ABC123/")
    add(con, "manual:2", "Analyst", fetched="2026-08-05", company="Northwind",
...
def test_three_rows_on_one_application_all_fold_onto_the_same_keeper(con):
    ...
    add(con, "li:1", "Support Analyst", fetched="2026-08-03", company="Northwind",
        apply_url="https://apply.workable.com/northwind/j/ABC123/")
    add(con, "workable:1", "Support Analyst", fetched="2026-08-01", company="Northwind",
        apply_url="https://apply.workable.com/northwind/j/ABC123")
    add(con, "greenhouse:1", "Support Analyst", fetched="2026-08-02", company="Northwind",
        apply_url="https://apply.workable.com/northwind/j/ABC123?utm_source=x")
...
def test_an_employers_own_careers_page_counts_as_its_application_page(con):
    """The third category: 31 rows across 24 single-tenant hosts, which no
...
def test_a_collected_ats_row_joins_the_linkedin_row_for_the_same_requisition(con):
    ...
    """
    sourced(con, "greenhouse:5500110022", "Community Manager", "greenhouse",
            "https://job-boards.greenhouse.io/harborcrest/jobs/5500110022",
            fetched="2026-08-01", company="Harborcrest Living")
    sourced(con, "manual:li-4400220011", "Community Manager", "imported",
            "https://www.linkedin.com/jobs/view/4400220011",
            apply_url="https://job-boards.greenhouse.io/harborcrest/jobs/5500110022",
            fetched="2026-08-02", company="Harborcrest Living")
...
def test_a_source_not_on_the_list_contributes_no_destination(con):
    ...
    """
    sourced(con, "aggregator:1", "Analyst", "driftboard",
            "https://www.driftboard.com/apply-with/1")
    sourced(con, "aggregator:2", "Analyst", "driftboard",
            "https://www.driftboard.com/apply-with/1")
    assert dupes.find(con) == [], \
...
def test_the_description_path_is_off_unless_asked_for(con):
    ...
    add(con, "wd:eastbranch", "Wealth Management Client Associate", shared)
    add(con, "wd:westbranch", "Wealth Management Client Associate", shared)
...
def test_a_job_already_applied_to_is_never_the_one_folded_away(con):
    """Decided 2026-08-09. Apply through the board row, then the LinkedIn row
    ...
    applications have been lost before.
    ...
    add(con, "li:1", "Support Analyst", fetched="2026-08-05", company="Northwind",
        apply_url="https://apply.workable.com/northwind/j/ABC123/")
...
def test_a_finished_application_still_counts_as_acted_on(con):
    ...
    add(con, "workable:1", "Analyst", fetched="2026-08-01", company="Northwind",
        apply_url="https://apply.workable.com/northwind/j/ABC")
    status.set_status(con, "workable:1", "applied")
    status.set_status(con, "workable:1", "no_offer")
    assert "no_offer" in status.FLOW, "written in the app's current vocabulary"
    add(con, "li:1", "Analyst", fetched="2026-08-05", company="Northwind",
        apply_url="https://apply.workable.com/northwind/j/ABC")
...
#
# Decided 2026-08-12: a seat re-advertised more than four weeks later is a new
# entry, linked back to the round it originated from rather than folded into it.
#
...
def add_dated(con, key, posted, apply_url, **kwargs):
    add(con, key, "Support Analyst", apply_url=apply_url, company="Northwind",
        **kwargs)
    con.execute("UPDATE jobs SET posted_at = ? WHERE key = ?", (posted, key))
    con.commit()


APPLY = "https://apply.workable.com/northwind/j/ABC123/"
...
def test_rows_with_no_posting_date_are_grouped_as_before(con):
    ...
    add(con, "workable:1", "Support Analyst", apply_url=APPLY, company="Northwind")
    add(con, "workable:2", "Support Analyst", apply_url=APPLY, company="Northwind")
