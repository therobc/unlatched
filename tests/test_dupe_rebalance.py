...
def pair(con, *, keeper_gone=None, hidden_gone=None):
    ...
    it existed to test. In life the group is made first and the sending
    collector pushes the closure the following day, which is what this now reproduces.
    """
...
def test_reopening_a_wrongly_closed_posting_restores_the_preference(con):
    ...
    A collector author deliberately does not act on inconclusive closure
    signals, but a false positive is still possible and `delist --back` is
    how they reverse one. The first version of rebalance only ever tested for "keeper closed,
