...
postings with full descriptions. The record SHAPE below is the one that
response uses - the field names, their types and the two id fields.

THE VALUES ARE INVENTED, and that is the point of the distinction. They were
verbatim from the live response to begin with: a real small business, a real
posting id and the slug built from its name, all republished here. A fixture
needs the shape to be right; it never needs somebody else's employer.
"""
...
POSTING = {
    "id": "1000001",
    "slug": "remote-support-analyst-fourth-coffee-1000001",
    ...
    "url": "https://remoteOK.com/remote-jobs/remote-support-analyst-1000001",
    "apply_url": "https://remoteOK.com/remote-jobs/remote-support-analyst-1000001",
}
...
def test_a_posting_maps_onto_the_shared_job_shape():
    ...
    assert job.source_id == "1000001"
    assert job.employer == "Fourth Coffee"
...
def test_the_company_name_is_html_unescaped():
    """The feed carries names encoded: an employer with an ampersand arrives
    as "&amp;", and was being stored and shown in the Companies table exactly
    like that. The description gets unescaped by html_to_text; the company
    name had nothing doing it."""
    record = dict(POSTING, company="Wingtip &amp; Sons")
    job = remoteok.collect({}, fetcher=make_fetcher(feed(LEGAL, record)))[0]
    assert job.employer == "Wingtip & Sons"
