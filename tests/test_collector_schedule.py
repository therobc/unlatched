...
def test_no_schedule_means_every_refresh(home):
    """THE LIVE PROFILE'S CASE. The first user's migrated handoff has no schedule, and it
    is pulled on every refresh today. A default of "on demand only" would stop
...
def test_asking_for_a_pull_ignores_the_schedule(home):
    """The first user's words: "pull from a fresh write on command". A schedule
    says when the app looks by ITSELF. Refusing a person who pressed the button
