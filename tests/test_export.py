...
def test_the_history_survives_not_just_the_current_status(con, cfg, tmp_path):
    ...
    before it ended - which is the whole record of the effort.

    Written in the app's CURRENT vocabulary. This spelled the last step
    "denied", which the app renamed to "no_offer" and now only migrates in
    from older databases - so it went on passing while exercising a word
    nothing produces. test_notes_and_offers covers the rename itself, which
    is where a retired spelling belongs.
    """
    key = add(con, cfg, "https://boards.greenhouse.io/acme/jobs/2")
    status.set_status(con, key, "applied")
    status.set_status(con, key, "interviewed")
    status.set_status(con, key, "no_offer")

    out = tmp_path / "pipeline.csv"
    export.write_csv(con, out)
    row = read_back(out)[0]

    assert row["status"] == "no_offer"
    for step in ("applied", "interviewed", "no_offer"):
        assert step in row["history"], f"{step} missing from {row['history']!r}"
    assert row["history"].index("applied") < row["history"].index("no_offer"), \
        "the order is the point - it is a history, not a set"


def test_a_finished_application_still_reports_when_it_was_applied_to(con, cfg, tmp_path):
...
def test_the_file_opens_with_accented_names_intact(con, cfg, tmp_path):
    ...
    out = tmp_path / "pipeline.csv"
    export.write_csv(con, out)
    assert read_back(out)[0]["company"] == "Crédit Vallée"
