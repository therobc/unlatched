...
so the only way to remove a row was by hand in the app. The question was
whether `forget-company` was the right instrument for 26 rows from
blacklisted reposters; it is not, and there was nothing else to point to.
...
def add(con, key, company="Driftboard", title="Support"):
...
def test_a_whole_employer_can_be_retired_in_one_call(con, home):
    """What a collector author needs: 26 rows across eight poster names,
    without sending 26 keys or reaching into the database."""
    add(con, "imported:1")
    add(con, "imported:2")
    add(con, "imported:3", company="Real Employer")
    con.commit()

    assert run(home, company="Driftboard") == 0
...
def test_it_goes_both_ways(con, home):
    ...
    run(home, company="Driftboard", back=True)
...
def test_a_dry_run_changes_nothing(con, home):
    ...
    assert run(home, company="Driftboard", dry_run=True) == 0
