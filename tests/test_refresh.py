...
postings land roughly 8:00-10:30 a.m.
...
def test_a_utc_stamp_is_converted_to_local_not_truncated():
    ...
    12:50 collection in a UTC-4 zone read as 16:50 and satisfy the 16:30 slot
...
def test_refresh_anchor_defaults_agree_across_all_three_copies():
    """The shipped default refresh times are written down three times.

    config.py's own comment calls its "at" default and refresh.DEFAULT_ANCHORS
    "two independent copies of the same decision" and warns that changing only
    one leaves them disagreeing. It undercounted: desktop/src/config.rs holds a
    third, and that one sat on 10:45 for weeks after the other two moved to
    11:00 - so a profile with no "at" key displayed one time on the Config
    screen and was collected at another.

    The Rust is READ AS TEXT rather than compiled. That is blunt, and it is the
    only way to compare a value across the two languages without a build step
    in the middle of a Python test - the same approach app.rs's own
    cache-invalidation tests take to their source.
    """
    import re
    ...
    from_config = config.DEFAULTS["refresh"]["at"]
    from_module = [f"{h:02d}:{m:02d}" for h, m in refresh.DEFAULT_ANCHORS]
    assert from_config == from_module, (
        f"config.py ships {from_config} but refresh.py schedules {from_module}")
    ...
    assert from_rust == from_config, (
        f"the desktop defaults to {from_rust} while the engine ships "
        f"{from_config}; a profile with no 'at' key would show one and "
        f"collect at the other")

    # THE WEEKEND TIME, THE SAME WAY. It is a third copy for the same reason
    # and drifts for the same reason - and it spent longer than the weekday one
    # not existing in the desktop at all.
    weekend_config = config.DEFAULTS["refresh"]["weekend_at"]
    weekend_module = [f"{h:02d}:{m:02d}" for h, m in refresh.WEEKEND_ANCHORS]
    assert weekend_config == weekend_module, (
        f"config.py ships {weekend_config} for the weekend but refresh.py "
        f"schedules {weekend_module}")

    weekend_block = re.search(r"weekend_at:\s*vec!\[([^\]]*)\]", rust)
    assert weekend_block, (
        "RefreshConfig::default no longer sets `weekend_at` - the Config "
        "screen cannot show a time the struct does not model, which is the "
        "state this field was added to correct")
    weekend_rust = re.findall(r'"([^"]+)"', weekend_block.group(1))
    assert weekend_rust == weekend_config, (
        f"the desktop defaults the weekend to {weekend_rust} while the engine "
        f"ships {weekend_config}")
