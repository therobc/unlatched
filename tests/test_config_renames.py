...
def test_a_config_that_never_had_any_of_them_is_untouched(home):
    ...
    assert cfg["fetch"][CURRENT] is True, "the shipped default is on"


def test_the_shipped_default_reads_added_links(home):
    """ON since 2026-09-07, and worth its own test rather than a line
    inside one about something else.

    The workflow it exists for: a person adds a link and the app fills the
    details in, then clicks re-check and is told whether the posting is still
    up. Both were built and both were dead while this shipped off.

    Defensible only because every read behind it is a click - `recheck` is
    reachable from the CLI subcommand, the Collect entry and the triage button,
    and neither refresh.py nor collectors.py calls it. If that ever stops being
    true, this default is the thing to reconsider first."""
    assert config.defaults()["fetch"]["read_added_links"] is True
