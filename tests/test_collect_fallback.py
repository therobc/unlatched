...
def test_company_with_nothing_still_skipped(tmp_path, monkeypatch, capsys):
    """An employer with no ATS fingerprint AND no careers page is passed over.

    ASSERTS THE SKIP, not just the silence. "No jobs appeared" is also what a
    run that never reached this employer looks like, and what a run that
    crashed before the loop looks like - so the summary is checked for the
    absence of an entry naming it, which is the thing that distinguishes
    being skipped from never being tried.
    """
    home = tmp_path / "home"
    con = db.connect(home)
