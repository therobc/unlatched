...
def add(con, key, *, last_seen=STALE, delisted=None, source=manual.SOURCE_NAME):
    ...
    db.upsert_job(con, key, {
        ...
        "url": f"https://www.linkedin.com/jobs/view/{key.split(':')[-1]}",
...
def test_nothing_with_a_status_is_re_read(con):
 ...
 action. Applied and passed were the first user's; denied he added; the rest follow
    from the same rule rather than from instruction."""
...
def test_imported_rows_are_never_re_read_by_this_button(con):
    ...
    have fetched were imported LinkedIn URLs, and not one was hand-added.
    """
    add(con, "manual:1")
...
def test_recheck_is_a_liveness_check_and_never_re_reads_the_details(con, cfg):
    """The question the first user asked: does a row that already HAS its description get
    it fetched again?
