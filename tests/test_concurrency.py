...
def test_without_a_timeout_a_blocked_write_fails_instantly(two_writer_db):
    ...
    # MEASURED AGAINST THE TIMEOUT, NOT AGAINST A CONSTANT. What this test knows
    # is that no waiting happened, and "no waiting" is only meaningful relative
    # to the wait it is being compared with. A fixed 0.1s budget is instead a
    # claim about the MACHINE, and can fail on a busy one while the code is
    # perfectly correct - the suite has had an unexplained intermittent failure
    # and this is the one assertion in it built to produce one.
    assert elapsed < (WAIT_MS / 1000) * 0.5, (
        f"failed after {elapsed:.3f}s, which is long enough to have been a wait")
