...
Field names below come from a live board (berkshiregrey), not from guesswork:
`datePosted`, `compensation`, `employmentStatusLabel`, `jobOpeningShareUrl`,
and an `atsLocation` that carries the city/state that `location` leaves null.
"""
...
LIST_PAYLOAD = {"result": [
    {"id": 645, "jobOpeningName": "Sales Executive", "isRemote": False,
     ...
     "atsLocation": {"city": "Bedford", "state": "Massachusetts"}},
]}

DETAIL_PAYLOAD = {"result": {"jobOpening": {
    "jobOpeningName": "Sales Executive",
    "datePosted": "2025-10-10",
    "description": "<p>Sell <b>robots</b>.</p><li>Travel 25%</li>",
    "compensation": "Base salary range $164k-$240k",
    "employmentStatusLabel": "Fulltime-Regular",
    "jobOpeningShareUrl": "https://berkshiregrey.bamboohr.com/careers/645",
}}}
...
def test_detail_call_supplies_date_description_and_pay():
    jobs = bamboohr.collect("berkshiregrey", fetcher=_fetcher())
    assert len(jobs) == 1
    job = jobs[0]
    assert job.posted == "2025-10-10"
    assert "Sell robots." in job.description
    assert job.employment_type == "Fulltime-Regular"
    assert job.url == "https://berkshiregrey.bamboohr.com/careers/645"
...
def test_ats_location_wins_when_location_is_all_nulls():
    jobs = bamboohr.collect("berkshiregrey", fetcher=_fetcher())
    assert jobs[0].location == "Bedford, Massachusetts"
...
def test_detail_fetches_are_bounded():
    big = {"result": [{"id": i, "jobOpeningName": f"Role {i}", "location": {},
                        "atsLocation": {"city": "Bedford", "state": "MA"}}
