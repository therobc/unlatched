...
def test_every_resolving_domain_is_not_probed(monkeypatch):
    ...
    discover.resolve("Fabrikam Logistics", fetcher=fetch)
...
def test_confirmation_stops_probing_other_domains(monkeypatch):
    monkeypatch.setattr(discover, "resolves", lambda *_a, **_k: True)
    confirming_page = (
        "<html><body>Careers at Fabrikam Logistics. " + "filler text " * 80
        + "</body></html>")
    fetch, calls = _counting_fetcher(
        {"https://www.fabrikamlogistics.com": confirming_page})

    result = discover.resolve("Fabrikam Logistics", fetcher=fetch)

    assert result["domain"] == "fabrikamlogistics.com"
    other_domain_calls = [c for c in calls if "fabrikamlogistics.com" not in c]
    assert other_domain_calls == []


def test_ats_fingerprint_still_wins_and_stops_early(monkeypatch):
    monkeypatch.setattr(discover, "resolves", lambda *_a, **_k: True)
    page = ("<html><body>Careers at Fabrikam Logistics "
             "<a href='https://boards.greenhouse.io/embed/job_board?for=fabrikam'>"
             "openings</a> " + "filler text " * 80 + "</body></html>")
    fetch, calls = _counting_fetcher({"https://careers.fabrikamlogistics.com": page})
