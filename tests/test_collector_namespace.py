...
def test_two_collectors_reporting_the_same_posting_do_not_overwrite_each_other(
        con, cfg):
    first = importer.import_row(con, cfg, dict(POSTING), collector="linkedin")
    second = importer.import_row(con, cfg, dict(POSTING), collector="indeed")
    ...
    assert len(rows) == 2
    assert {r["source"] for r in rows} == {"linkedin", "indeed"}
...
def test_a_collector_cannot_claim_another_collectors_namespace(con, cfg):
    ...
    A file that supplies "indeed:123" while being imported as the linkedin
    collector is trying, deliberately or not, to write into indeed's rows.
    """
    row = dict(POSTING, key="indeed:123")
    stored = importer.import_row(con, cfg, row, collector="linkedin")

    # KEPT WHOLE INSIDE linkedin's NAMESPACE rather than rewritten to
    # "linkedin:123": the sender's id is theirs and may have structure, and
    # what matters here is only that it cannot land in indeed's rows.
    assert stored["key"] == "linkedin:indeed:123"
    assert not stored["key"].startswith("indeed:")
...
def test_two_ids_that_differ_only_by_the_senders_own_prefix_stay_two_rows(
    ...
    first = importer.import_row(
        con, cfg, dict(POSTING, key="li:998877"), collector="indeed")
...
def test_case_is_normalised_because_config_is_written_by_hand(con, cfg):
    """"LinkedIn" is what a person types. It is accepted and lowercased, and
    ...
    stored = importer.import_row(con, cfg, dict(POSTING), collector="LinkedIn")

    assert stored["key"].startswith("linkedin:")
    source = con.execute("SELECT source FROM jobs WHERE key = ?",
                         (stored["key"],)).fetchone()[0]
    assert source == "linkedin"
...
def test_a_sender_still_using_the_old_prefix_updates_the_row_it_already_wrote(
 ...
 rekey.py corrected 410 rows from `manual:` to `imported:` on
    ...
    already_here = importer.import_row(
        con, cfg, dict(POSTING, key="imported:www-linkedin-com-jobs-view-4435183193"))

    from_the_sender = importer.import_row(
        con, cfg, dict(POSTING, key="manual:www-linkedin-com-jobs-view-4435183193",
...
def test_a_batch_carries_its_collector_to_every_row(con, cfg):
    rows = [dict(POSTING, url=f"https://example.invalid/{n}", title=f"Role {n}")
            for n in range(3)]
    result = importer.import_all(con, cfg, rows, collector="indeed")

    assert result["imported"] == 3
    sources = {r[0] for r in con.execute("SELECT DISTINCT source FROM jobs")}
    assert sources == {"indeed"}
...
def test_re_importing_the_same_collectors_run_updates_rather_than_duplicates(
    ...
    importer.import_row(con, cfg, dict(POSTING), collector="linkedin")
