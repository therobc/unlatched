...
The second is a defect that looks exactly like the first. The collector author hit it on her
own side - a silent empty write - and three runs of rows reached this database
unjoinable with nothing to say so. She then asked to be told if a row ever
arrives with neither a destination nor a classification, which could not be done
here at all: there was nowhere for a classification to live.
"""
from __future__ import annotations

from unlatched import db, importer

BASE = {"url": "https://www.linkedin.com/jobs/view/1", "title": "Analyst",
        "company": "Acme"}


def test_a_stated_easy_apply_is_recorded_as_one(con, cfg):
    importer.import_row(con, cfg, dict(BASE, apply_kind="easy-apply"))
    row = db.get_job(con, "imported:www-linkedin-com-jobs-view-1")
...
def test_a_marker_in_the_destination_is_caught_and_reported(con, cfg):
    """Her contract: 'easy-apply' and 'closed' are classifications, NEVER URLs.
    ...
    result = importer.import_all(con, cfg, [
        dict(BASE, url="https://www.linkedin.com/jobs/view/1", apply_url="easy-apply"),
        dict(BASE, url="https://www.linkedin.com/jobs/view/2", apply_url="easy-apply"),
    ])

    assert len(result["markers_in_apply_url"]) == 2
    assert result["imported"] == 2, "the jobs are still imported - one bad field"
    for n in (1, 2):
        row = db.get_job(con, f"imported:www-linkedin-com-jobs-view-{n}")
...
def test_rows_with_neither_a_destination_nor_a_classification_are_counted(con, cfg):
    """The number she asked for, which nothing could produce before."""
    result = importer.import_all(con, cfg, [
        dict(BASE, url="https://www.linkedin.com/jobs/view/1"),
        dict(BASE, url="https://www.linkedin.com/jobs/view/2", apply_kind="easy-apply"),
        dict(BASE, url="https://www.linkedin.com/jobs/view/3",
...
def test_easy_apply_rows_still_never_match_each_other(con, cfg):
    ...
    importer.import_all(con, cfg, [
        dict(BASE, url="https://www.linkedin.com/jobs/view/1", apply_kind="easy-apply"),
        dict(BASE, url="https://www.linkedin.com/jobs/view/2", apply_kind="easy-apply"),
    ])
