...
purpose. It does not read the aggregators - Indeed, Glassdoor, LinkedIn - and
it is not going to.
...
| `location` | no | Free text: `Remote, US`, `Dayton, OH`. |
...
| `status` | no | What the person already did: `applied`, `pass`, `no_offer`... |
...
`myboard`, your `998877` becomes `myboard:998877`. Two collectors reporting
...
the `myboard` collector carrying `othertool:123` becomes
`myboard:othertool:123` - unambiguous, and unable to collide with anything of
`othertool`'s. The only prefix that is replaced rather than kept is your own
id, so sending `myboard:998877` and `998877` mean the same row.
...
Everything above runs one way. One fact runs the other way.

A person using Unlatched opens a posting, reads "no longer accepting
applications", and marks it taken down. That is the fastest closure signal in
the system, and unless it is handed back it exists only here - your collector
keeps the lead on its own board and goes on spending sweep budget re-checking
something that is already gone.

**A file, not a write into your store.** The same reasoning as the incoming
direction: you own your data, a file is inspectable, and neither side has to be
running when the other is. It is written beside the file you hand over, with
`-closed-by-unlatched.json` in place of your file's extension - hand over
`inbox.json` and you are handed back `inbox-closed-by-unlatched.json`.
...
```json
{
  ...
  "source": "unlatched",
  "collector": "myboard",
  "generated_at": "2026-08-26T09:14:03-04:00",
  "closed": [
    {
      "key": "myboard:1042",
      "url": "https://example.com/careers/1042",
      "title": "Staff Engineer",
      "closed_at": "2026-08-25T13:02:00+00:00"
    }
  ]
}
```
...
| Field | What it is |
|---|---|
| `key` | The posting's key **in your namespace**, not this app's. |
| `url` | The posting URL. The reliable join; the key is the convenience. |
| `title` | What the posting was called, so the file reads to a human. |
| `closed_at` | When the closure was recorded, with an explicit UTC offset. |

**Your keys, not ours.** If your rows arrive under a prefix this app has since
renamed, the hand-back spells them the way *you* spell them. Handing back a key
you do not recognise is how these closures failed to travel in the first place.

**Everything it holds as closed, not only what a person marked.** Provenance is
not recorded - a closure you pushed and one a person marked are the same fact
by the time they get here - and it does not need to be. The signal is "this app
believes this posting is closed", and anything you already knew costs you one
line to skip.

**Closed, never removed.** Somebody retiring a row is taking it off their own
lists: a decision about their search, and none of your business. Being closed
is a fact about the world and belongs to both sides.

**Written whole, then moved into place**, so a reader looking at any moment
sees either the previous file or the new one, never half of one.

**It is a courtesy, so it never fails a run.** A path that cannot be written is
skipped in silence. A refresh that collected successfully does not report
failure because this file could not be saved.

One is written for every enabled collector at the end of each refresh, or on
demand:
...
"collectors": [
  {
    "id": "myboard",
    "label": "My board collector",
    "path": "C:/Users/you/collectors/myboard/handoff.csv",
    "enabled": true,
    "schedule": ["13:00"],
...
`id` is 1-32 characters of `a-z`, `0-9`, underscore or hyphen, and must start
with a letter or a digit. It becomes both the row namespace and the provenance
shown on screen, so it cannot contain a colon.

**Pulling on demand.** A person can pull at any time from **Collect -> From a
collector** on the Companies screen, or:

```
unlatched ingest --collector myboard       # just this one
...
* **`we_may_refetch: true` cannot widen what Unlatched fetches.** It is a
  ...
  everybody, including the person. Naming your collector after a blocked site
  gains you nothing - the host decides, and the host has not changed.
